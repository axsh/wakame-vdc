  ...
  module Models
    class AccountLog < Sequel::Model
      ...
      def self.generate(year, month)
        ...
        instances = {}
        before_run_instance_uuid(year, month).merge(instance_uuids(year, month)).each{|k,v|
          logs = instance_logs(year, month, v.target_uuid).map{|i|
