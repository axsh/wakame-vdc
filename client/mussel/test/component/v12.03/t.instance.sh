...
function test_instance_index_stateless() {
  ...
  assertEquals "$(cli_wrapper ${namespace} ${cmd})" \
               "curl -X GET $(base_uri)/${namespace}s.${DCMGR_RESPONSE_FORMAT}?service_type=std"
}
...
function test_instance_index_stateful() {
  ...
  assertEquals "$(cli_wrapper ${namespace} ${cmd} --state=${state})" \
               "curl -X GET $(base_uri)/${namespace}s.${DCMGR_RESPONSE_FORMAT}?service_type=std&state=${state}"
}
...
function test_instance_backup() {
  ...
  assertEquals "$(cli_wrapper ${namespace} ${cmd} ${uuid})" \
               "curl -X PUT $(urlencode_data ${params}) $(base_uri)/${namespace}s/${uuid}/${cmd}.${DCMGR_RESPONSE_FORMAT}"
}
