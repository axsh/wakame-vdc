...
declare namespace=backup_object
...
function test_backup_object_help_stderr_to_devnull_success() {
...
function test_backup_object_index() {
...
function test_backup_object_show_no_uuid() {
  extract_args ${namespace} show
  run_cmd ${MUSSEL_ARGS} 2>/dev/null
  assertNotEquals $? 0
}

function test_backup_object_show_uuid() {
...
function test_backup_object_destroy() {
...
function test_backup_object_xcreate() {
