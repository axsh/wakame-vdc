...
function test_storage_node_help_stderr_to_devnull_success() {
...
function test_storage_node_help_stderr_to_stdout_success() {
  ...
  assertEquals "${res}" "$0 ${namespace} [help|index|show|destroy]"
}
...
function test_storage_node_show_no_uuid() {
  extract_args ${namespace} show
  run_cmd ${MUSSEL_ARGS} 2>/dev/null
  assertNotEquals $? 0
}

function test_storage_node_show_uuid() {
