...
Sequel.migration do
  ...
  module MigrateLocalStoreToVolume
    def self.data_migrate
      ...
      db[:instances].filter(:state=>'running').each { |instance|
        image = db[:images][:id=>instance[:image_id]]
        next if image[:boot_dev_type] != Dcmgr::Constants::Image::BOOT_DEV_LOCAL
      ...
      db[:instances].filter(:state=>'running').each { |instance|
        image = db[:images][:id=>instance[:image_id]]
        next if image[:boot_dev_type] != Dcmgr::Constants::Image::BOOT_DEV_SAN
