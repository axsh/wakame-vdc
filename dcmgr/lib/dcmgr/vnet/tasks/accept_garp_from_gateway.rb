    ...
    module Tasks
      ...
      class AcceptGARPFromGateway < Task
        ...
        def initialize(gw_ip,enable_logging = false,log_prefix = nil)
          super()
          self.gw_ip = gw_ip
          self.rules << EbtablesRule.new(:filter,:forward,:arp,:incoming,"--protocol arp --arp-mac-dst=ff:ff:ff:ff:ff:ff --arp-ip-src=#{self.gw_ip} --arp-ip-dst=#{self.gw_ip} #{EbtablesRule.log_arp(log_prefix) if enable_logging} -j ACCEPT")
        end
      end
