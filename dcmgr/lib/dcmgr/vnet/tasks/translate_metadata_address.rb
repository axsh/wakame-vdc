    ...
    module Tasks
      ...
      class TranslateMetadataAddress < Task
        ...
        def initialize(metadata_ip,metadata_port,metadata_fake_ip = "169.254.169.254",metadata_fake_port = "80")
          ...
          self.rules << IptablesRule.new(:nat,:prerouting,:tcp,:outgoing,"-d #{self.metadata_fake_ip} -p tcp --dport #{self.metadata_fake_port} -j DNAT --to-destination #{self.metadata_ip}:#{self.metadata_port}")
