  ...
  post do
    ...
    begin
      vdcs = M::VirtualDataCenterSpec.entry_new(@account) do |spec|
        ...
        when Hash
          spec.display_name = file['display_name']
          spec.file = file
        else
          raise E::InvalidParameter, params['file']
        end
      end
    rescue M::VirtualDataCenterSpec::YamlLoadError, Sequel::ValidationFailed, Sequel::InvalidValue => e
  ...
  put '/:id' do
    ...
    if params['file']
      begin
        case file = M::VirtualDataCenterSpec.load(params['file'])
        when Hash
          vdcs.display_name = file['display_name']
          vdcs.file = file
          vdcs.save_changes
        else
          raise E::InvalidParameter, params['file']
