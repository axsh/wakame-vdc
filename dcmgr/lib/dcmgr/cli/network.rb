...
class Network < Base
  ...
  no_tasks {
    ...
    Dcmgr::Configurations::Dcmgr.include_helper(self.class)
  ...
  method_option :service_type, :type => :string, :default=>dcmgr_conf.default_service_type, :desc => "Service type of the network. (#{dcmgr_conf.service_types.keys.sort.join(', ')})"
  ...
  method_option :service_type, :type => :string, :desc => "Service type of the network. (#{dcmgr_conf.service_types.keys.sort.join(', ')})"
...
__END
  ...
  class PoolOps < Base
    ...
    def acquire(pool_uuid)
      ...
      st = Dcmgr::Scheduler.service_type(dcmgr_conf.default_service_type)
