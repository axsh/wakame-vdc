...
module Dcmgr::Cli
  class Spec < Base
    ...
    method_option :memory_size, :type => :numeric, :aliases => "-m", :default => 1000, :desc => "The memory size for the new instance."
...
__END
        ...
        print ERB.new(<<__END, nil, '-').result(binding)
<%- specs.each { |row| -%>
<%= "%-20s  %-15s %-15s %-15s" % [row.canonical_uuid, row.account_id, row.arch] %>
<%- } -%>
__END
      end
    end
