              ...
              resources.each {|k, v|
                ...
                if alarm.evaluate?
                  if @alarm_manager.evaluate(alarm.uuid)
                    alarm.update_state(ALARM_STATE)
                    data = alarm.to_hash.merge({"ipaddr"=>v["ipaddr"]})
                    action = data["alarm_actions"]
                    message = Notification.build_message(action, data)
                    logger.info("Send Notification #{action["notification_id"]}")
                    Notification.send(message)
                  else
                    alarm.update_state(OK_STATE)
                    data = alarm.to_hash.merge({"ipaddr"=>v["ipaddr"]})
                    action = data["ok_actions"]
                    message = Notification.build_message(action, data)
                    logger.info("Send Notification #{action["notification_id"]}")
                    Notification.send(message)
                  end
