...
module Dcmgr
  module NodeModules
    class StaTgtInitializer < Isono::NodeModules::Base
      ...
      initialize_hook do
        if manifest.config.initiator_address.nil?
          abort("initiator_address is not set yes in sta-tgt.conf")
        end
        
        unless manifest.config.backing_store == 'tgt'
          abort('Please set "config.iscsi_target = \'tgt\'" in sta.conf')
        end

        unless manifest.config.iscsi_target == 'linux_iscsi'
          abort('Please set "config.backing_store = \'linux_iscsi\'" in sta.conf')
        end
      ...
      def register_volumes
        ...
        volumes.each { |volume|
          ...
          if volume[:storage_pool][:storage_type] == 'tgt'
