...
module Dcmgr
  module NodeModules
    class JobQueueWorker < Isono::NodeModules::Base
      ...
      def subscribe(queue_name, concurrency, default_retry_max, &blk)
        return if @pollers.has_key?(queue_name)

        poller = @pollers[queue_name] = Poller.new(queue_name, concurrency, self, default_retry_max, blk)
      ...
      class Poller
        ...
        def initialize(queue_name, concurrency, worker, default_retry_max, blk)
        ...
        def retry_max(job)
          # Use default number which is set to Queue Poller if :retry_max is zero.
          job[:retry_max].to_i > 0 ? job[:retry_max].to_i : @default_retry_max
        end
        def pop_once
          job = pop_job
          if job.nil?
            ...
            begin
              ...
              if job[:retry_count] < retry_max(job)
                logger.info("Running Job Failed. Re-queueing. (retry current #{job[:retry_count]} < max #{retry_max(job)}) : #{job[:uuid]}: #{e.message}")
              else
                logger.info("Running Job Failed. Giving up. (retried #{retry_max(job)} times): #{job[:uuid]}: #{e.message}")
              end
