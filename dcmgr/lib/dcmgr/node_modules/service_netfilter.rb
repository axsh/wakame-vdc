  ...
  module NodeModules
    ...
    class ServiceNetfilter < Isono::NodeModules::Base
      ...
      initialize_hook do
        ...
        if Dcmgr.conf.netfilter_persist
          @task_manager.netfilter_start_script = Dcmgr.conf.netfilter_persist_script_path
          @task_manager.iptables_save_file = Dcmgr.conf.netfilter_persist_iptables_file
          @task_manager.ebtables_save_file = Dcmgr.conf.netfilter_persist_ebtables_file
        end
        ...
        @worker_thread.pass {
          ...
          (@cache.get_all_local_vnics + @cache.get_all_empty_vnics).each { |vif|
            ...
            @task_manager.apply_vnic_tasks(vif,TaskFactory.create_tasks_for_vnic(vif,friends,security_groups,node))
          ...
          @cache.get_all_security_groups.each { |group|
            ...
            group[:referencees].each { |ref|
              myinstance.subscribe_to_referencees(ref[:uuid])
            }
          }
        ...
        myinstance.subscribe_to_event("hva.#{myinstance.node.manifest.node_instance_id}/vnic_created") do |args|
          @worker_thread.pass {
            ...
            @task_manager.apply_vnic_tasks(vnic,TaskFactory.create_tasks_for_vnic(vnic,friends,security_groups,node))
            
            # Add isolation tasks to friend vnics
            vnic[:security_groups].each { |group_id|
              @cache.get_all_local_friends(vnic[:uuid]).each { |friend|
                @task_manager.apply_vnic_tasks(friend,TaskFactory.create_tasks_for_isolation(friend,[vnic],node))
              }
            }
          ...
          @worker_thread.pass {
            ...
            vif[:security_groups].each { |group_id|
              unless @cache.other_local_vnics_left_in_group?(vnic_id,group_id)
                ...
                group[:referencees].each { |ref|
                  ...
                  next if @cache.other_groups_referencing_group?(group_id,ref[:uuid])
                  
                  myinstance.unsubscribe_from_referenced_security_group(ref[:uuid])
                }
              end
            }
            ...
            if @cache.vnic_has_ip?(vnic_id)
              ...
              local_friends.each { |friend|
                ...
                @task_manager.remove_vnic_tasks(friend,TaskFactory.create_tasks_for_isolation(friend,[vif],self.node))
              }
              ...
              if @cache.vnic_is_natted?(vnic_id)
                ...
                @task_manager.remove_vnic_tasks(vif,TaskFactory.create_tasks_for_isolation(vif,all_friends,self.node))
              end
              
              # Removing this outside of the is_natted? check because it includes
              # the address translation for metadata server.
              @task_manager.remove_vnic_tasks(vif, TaskFactory.create_nat_tasks_for_vnic(vif,self.node) )
            end
      ...
      def subscribe_to_local_vnic(vnic_id)
        ...
        subscribe_to_event("#{vnic_id}/joined_group") do |args|
          @worker_thread.pass {
            ...
            unless vnic_map.nil?
              @task_manager.apply_vnic_tasks(vnic_map,TaskFactory.create_tasks_for_secgroup(group_map))
              ...
              local_friends.each { |local_vnic_map|
                @task_manager.apply_vnic_tasks(local_vnic_map,TaskFactory.create_tasks_for_isolation(local_vnic_map,[vnic_map],self.node))
              }
              
              # Add isolation rules to self
              friends = @cache.get_friends_in_group(vnic_id,group_id)
              @task_manager.apply_vnic_tasks(vnic_map,TaskFactory.create_tasks_for_isolation(vnic_map,friends,self.node))
            end
        ...
        subscribe_to_event("#{vnic_id}/left_group") do |args|
          @worker_thread.pass {
            ...
            unless vnic_map.nil?
              @task_manager.remove_vnic_tasks(vnic_map,TaskFactory.create_tasks_for_secgroup(group_map))
              
              local_friends = @cache.get_local_friends_in_group(vnic_id,group_id)
              
              # Remove isolation rules from local friends
              local_friends.each { |local_vnic_map|
                @task_manager.remove_vnic_tasks(local_vnic_map,TaskFactory.create_tasks_for_isolation(local_vnic_map,[vnic_map],self.node))
              }
              ...
              @task_manager.remove_vnic_tasks(vnic_map,TaskFactory.create_tasks_for_isolation(vnic_map,friends,self.node))
      ...
      def subscribe_to_referencees(ref_group_id)
        subscribe_to_event("#{ref_group_id}/vnic_joined") do |args|
          @worker_thread.pass {
            ...
            for i in 1..5 do
              @cache.update
              break if @cache.is_foreign_vnic_in_group?(vnic_id,ref_group_id) || @cache.is_local_vnic?(vnic_id) || @cache.is_referencee_vnic_in_group?(vnic_id,ref_group_id)
              sleep 1
            end
            ...
            if @cache.is_foreign_vnic_in_group?(vnic_id,ref_group_id)
              ...
              local_friends.each { |local_vnic_map|
                @task_manager.apply_vnic_tasks(local_vnic_map,TaskFactory.create_tasks_for_isolation(local_vnic_map,[foreign_vnic_map],self.node))
              }
            end
            ...
            @cache.get_all_security_groups.each { |secg_map|
              ...
              lvnics = @cache.get_local_vnics_in_group(ref[:uuid])
              referencer_vnic_map = ref[:vnic].find {|v| v[:uuid] == vnic_id}
              lvnics.each { |lvnic|
                @task_manager.apply_vnic_tasks(vnic_id,TaskFactory.create_tasks_for_ARP_isolation(lvnic,[referencer_vnic_map],node))
              }
            }
        ...
        subscribe_to_event("#{ref_group_id}/vnic_left") do |args|
          @worker_thread.pass {
            ...
            if @cache.is_foreign_vnic_in_group?(vnic_id,ref_group_id)
              ...
              local_friends.each { |local_vnic_map|
                @task_manager.remove_vnic_tasks(local_vnic_map,TaskFactory.create_tasks_for_isolation(local_vnic_map,[foreign_vnic_map],self.node))
              }
            end
      ...
      def subscribe_to_security_group(group_id)
        ...
        subscribe_to_event("#{group_id}/referencer_added") do |args|
          @worker_thread.pass {
            ...
            @cache.get_local_vnics_in_group(group_id).each { |lvnic|
              @task_manager.apply_vnic_tasks(lvnic,TaskFactory.create_tasks_for_ARP_isolation(lvnic,referencer[:vnics],node))
            }
        ...
        subscribe_to_event("#{group_id}/referencer_removed") do |args|
          @worker_thread.pass {
            ...
            @cache.get_local_vnics_in_group(group_id).each { |lvnic|
              @task_manager.remove_vnic_tasks(lvnic,TaskFactory.create_tasks_for_ARP_isolation(lvnic,referencer[:vnics],node))
            }
        ...
        subscribe_to_event("#{group_id}/rules_updated") do |args|
          @worker_thread.pass {
            ...
            old_referencee_uuids = @cache.get_group(group_id)[:referencees].map { |r| r[:uuid] }
            ...
            updated_referencee_uuids = @cache.get_group(group_id)[:referencees].map { |r| r[:uuid] }
            
            @cache.get_local_vnics_in_group(group_id).each { |vnic|
              # Add the new security group tasks
              @task_manager.apply_vnic_tasks(vnic, TaskFactory.create_tasks_for_ARP_isolation(vnic,updated_referencee_vnics,node))
              @task_manager.apply_vnic_tasks(vnic, TaskFactory.create_tasks_for_secgroup(updated_group))
              
              # Remove the old security group tasks
              @task_manager.apply_vnic_tasks(vnic,  TaskFactory.create_tasks_for_ARP_isolation(vnic,old_referencee_vnics,node))
              @task_manager.remove_vnic_tasks(vnic, TaskFactory.create_tasks_for_secgroup(old_group))
            }
      ...
      def handle_ref_group_vnic(group_id,vnic_id,action = :apply)
        ...
        @cache.get_all_security_groups.each { |secg_map|
          ...
          vnics.each { |vnic_map|
            ...
            @task_manager.send("#{action}_vnic_tasks", vnic_map, TaskFactory.create_tasks_for_ARP_isolation(vnic_map,[referencer_vnic_map],node))
            @task_manager.send("#{action}_vnic_tasks", vnic_map, TaskFactory.create_tasks_for_secgroup(dummy_group))
          }
        }
      end
