    ...
    module NetFramework
      ...
      class IptablesRule < Rule
        ...
        def initialize(table = nil, chain = nil, protocol = nil, bound = nil, rule = nil)
          super()
          raise ArgumentError, "table does not exist: #{table}" unless NetFramework::IptablesPreMadeChains.keys.member?(table)
          self.table = table
          self.chain = chain
        ...
        def chain
          if NetFramework::IptablesPreMadeChains[self.table].member?(@chain)
            @chain.to_s.upcase 
          else
            @chain
          end
      ...
      class EbtablesRule < Rule
        ...
        def initialize(table = nil, chain = nil,  protocol = nil, bound = nil, rule = nil)
          super()
          raise ArgumentError, "table does not exist: #{table}" unless NetFramework::EbtablesPreMadeChains.keys.member?(table)
          self.table = table
          self.chain = chain
          self.protocol = protocol
          self.bound = bound
        ...
        def chain
          if NetFramework::EbtablesPreMadeChains[self.table].member?(@chain)
            @chain.to_s.upcase 
          else
            @chain
          end
      ...
      class AcceptIpFromSameGroup < Task
      ...
      class AllowARPFromSameGroup < Task
        attr_reader :inst_ip
        attr_reader :friend_ips
        attr_reader :enable_logging
        attr_reader :log_prefix
      ...
      class AllowWakameDHCPOnly < Task
      ...
      class IptablesChain < Chain
        def initialize(table,name)
          raise ArgumentError, "table #{table} doesn't exist. Existing tables are '#{NetFramework::IptablesPreMadeChains.keys.join(",")}'." unless NetFramework::IptablesPreMadeChains.keys.member?(table)
          raise ArgumentError, "name can not be any of the following: '#{NetFramework::IptablesPreMadeChains[table].join(",")}'." if NetFramework::IptablesPreMadeChains[table].member?(name)
      ...
      class EbtablesChain < Chain
        def initialize(table,name)
          raise ArgumentError, "table #{table} doesn't exist. Existing tables are '#{NetFramework::EbtablesPreMadeChains.keys.join(",")}'." unless NetFramework::EbtablesPreMadeChains.keys.member?(table)
          raise ArgumentError, "name can not be any of the following: '#{NetFramework::EbtablesPreMadeChains[table].join(",")}'." if NetFramework::EbtablesPreMadeChains[table].member?(name)
          
          super
        end
      end

      # Abstract class that determines how to isolate instances from each other
      class Isolator
      ...
      class TaskFactory
        ...
        def self.create_tasks_for_vnic(vnic,friends,node)
          ...
          if is_natted? vnic
            ...
            if ipset_enabled
              tasks << ExcludeNatIpSet(friend_ips,vnic[:ipv4][:address])
            else
              tasks << ExcludeNat(friend_ips,vnic[:ipv4][:address])
            end
          end
          
          # General ebtables tasks
          tasks << AllowArpBroadcast.new(host_addr,enable_logging,"A arp bc #{vnic[:uuid]}: ")
          tasks << AntiIpSpoofing.new(vnic[:ipv4][:address],enable_logging,"D arp sp #{vnic[:uuid]}: ")
          tasks << AntiMacSpoofing.new(clean_mac(vnic[:mac_addr]),enable_logging,"D ip sp #{vnic[:uuid]}: ")
          tasks << AllowARPToHost.new(host_addr,vnic[:ipv4][:address],enable_logging,"A arp to_host #{vnic[:uuid]}: ")
          
          # General iptables tasks
          tasks << AcceptIcmpRelatedEstablished.new
          tasks << AcceptTcpRelatedEstablished.new
          tasks << AcceptUdpRelatedEstablished.new
          #tasks << AllowWakameDNSOnly.new(vnic[:ipv4][:network][:dns_server])
          tasks << AllowAllDNS.new
          tasks << AllowWakameDHCPOnly.new(vnic[:ipv4][:network][:dhcp_server])
          ...
          tasks << AllowOutgoingIpTraffic.new
          
          # Security group tasks
          #tasks << SecurityGroup(....)
          
          # VM isolation based on same security group
          #tasks << AllowARPFromSameGroup.new(vnic[:ipv4][:address],friend_ips,enable_logging,"A arp friend #{vnic.uuid}") # <--- constructor values not filled in yet
          #tasks << AcceptIpFromSameGroup(friend_ips)
          
          # Accept traffic from the gateway that isn't blocked by other tasks
          ...
          tasks << DropIncomingIpTraffic.new #<-- has no arguments
      ...
      class NetfilterController < NetFramework::Controller
    ...
    class ServiceNetfilter < Isono::NodeModules::Base
      ...
      initialize_hook do
        ...
        @worker_thread.pass {
          ...
          myinstance.controller = NetFramework::ControllerFactory.create_controller(myinstance.node)
        }
