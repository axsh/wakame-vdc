    ...
    module NewDesign
      ...
      class AllowARPToHost < Task
        ...
        def initialize(host_ip,inst_ip,enable_logging,log_prefix)
          ...
          self.rules << EbtablesRule.new(:filter,:input,:arp,:outgoing,"--protocol arp --arp-ip-src #{self.self_ip} --arp-ip-dst #{self.host_ip} --log-ip --log-arp --log-prefix '#{self.log_prefix}' -j CONTINUE") if self.enable_logging
          self.rules << EbtablesRule.new(:filter,:input,:arp,:outgoing,"--protocol arp --arp-ip-src #{self.self_ip} -j ACCEPT")
        end
      end
    ...
    class ServiceNetfilter < Isono::NodeModules::Base
      ...
      def build_rule(rules = [])
        ...
        rules.each do |rule|
          ...
          # protocol    : [ ip4 | ip6 | netfilter_group_uuid ]
          ...
          begin
            ...
            until s.eos?
              ...
              when s.scan(/ng-\w+/)
                from_group = true
                inst_maps = rpc.request('hva-collector', 'get_instances_of_netfilter_group', protocol)
                inst_maps.each { |inst_map|
                  ipv4s << inst_map[:ips]
                }
              else
