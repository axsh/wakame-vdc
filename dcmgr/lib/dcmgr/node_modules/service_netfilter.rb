    ...
    class ServiceNetfilter < Isono::NodeModules::Base
      ...
      def subscribe_to_security_group(group_id)
        subscribe_to_event("#{group_id}/vnic_joined") do |args|
          @worker_thread.pass {
            logger.debug "event caught: #{group_id}/vnic_joined"
            vnic_id = args[0]
            #myinstance.controller.join_security_group(vnic_id,group_id)
          }
        end
        
        subscribe_to_event("#{group_id}/vnic_left") do |args|
          @worker_thread.pass {
            logger.debug "event caught: #{group_id}/vnic_left"
            vnic_id = args[0]
            #myinstance.controller.leave_security_group(vnic_id,group_id)
          }
        end
        ...
        subscribe_to_event("#{group_id}/rules_updated") do |args|
          @worker_thread.pass {
            ...
            myinstance.controller.update_security_group(group_id)
          }
        end
      end

      def unsubscribe_from_security_group(group_id)
        unsubscribe_from_event("#{group_id}/instance_joined")
        unsubscribe_from_event("#{group_id}/instance_left")
