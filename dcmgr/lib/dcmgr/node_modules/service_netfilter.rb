...
module Dcmgr
  module NodeModules
    ...
    class ServiceNetfilter < Isono::NodeModules::Base
      ...
      # It is called when deleting an instance.
      ...
      def build_iptables_chains(protocol_map, vif_map, inst_map, action = :create)
        actions = { :create => ['N'] , :delete => ['F','X'] }
        raise ArgumentError, "#{action} is not a valid action. Valid actions are #{actions.keys.join(',')}" unless actions.keys.member?(action)
      ...
      def build_iptables_basic_part(vif_map, inst_map, action = :create)
        ...
        # make chain names.
        basic_cmds << build_iptables_chains(protocol_map, vif_map, inst_map, :create) if action == :create
        ...
        basic_cmds << build_iptables_chains(protocol_map, vif_map, inst_map, :delete) if action == :delete
      ...
      def build_iptables_final_part(vif_map)
        ...
        [ 'd' ].each do |bound|
          protocol_map.each { |k,v|
            ...
            final_cmds << "iptables -A #{bound}_#{vif_map[:uuid]} -p #{k} -j #{bound}_#{vif_map[:uuid]}_#{k}_drop"
          }
        end
        ...
        [ 's' ].each do |bound|
          protocol_map.each { |k,v|
            final_cmds << "iptables -A #{bound}_#{vif_map[:uuid]}_#{k} -p #{k} -j ACCEPT"
          }
        end
