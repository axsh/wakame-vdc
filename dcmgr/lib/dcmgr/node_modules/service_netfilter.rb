  ...
  module NodeModules
    ...
    module Bandwidth
      ...
      def clear_bandwidth_limits
        ...
        "tc qdisc del dev #{find_nic(Dcmgr.conf.hv_ifindex)} root"
      end
      ...
      def limit_bandwidth(networks)
        ...
        nic = find_nic(Dcmgr.conf.hv_ifindex)
      ...
      def nat_instance(inst_map)
        ...
        inst_map[:instance_nics].each { |nic|
          ...
          nat_ips.each { |external_ip|
            ...
            nat_cmd << postrouting_command  + " -j LOG --log-prefix 'Snat '" if Dcmgr.conf.packet_drop_log
            nat_cmd << postrouting_command  + " -j SNAT --to #{external_ip.address}"

            nat_cmd << prerouting_command + " -j LOG --log-prefix 'Dnat '" if Dcmgr.conf.packet_drop_log
    ...
    class ServiceNetfilter < Isono::NodeModules::Base
      ...
      initialize_hook do
        ...
        if Dcmgr.conf.edge_networking == 'legacy_netfilter'
          ...
          @worker_thread.pass {
            ...
            myinstance.controller = VNet::ControllerFactory.create_controller(myinstance.node,event)
            myinstance.controller.event = event
            ...
            # Subscribe to referenced security groups
            myinstance.controller.get_security_groups.each { |group|
              group[:rules].each { |rule|
                ref_group_id = rule[:ip_source].scan(/sg-\w+/).first
                next if ref_group_id.nil?
                myinstance.subscribe_to_referenced_security_group(ref_group_id)                
              }
            }
          }
            ...
            @worker_thread.pass {
              ...
              inst[:vif].each { |vif|
                vif[:security_groups].each { |secg|
                  ...
                  # Subscribe to referenced security groups
                  #TODO: do this with referenced groups array instead of a regexp
                  myinstance.controller.get_security_groups(secg)[:rules].each { |rule|
                    ref_group_id = rule[:ip_source].scan(/sg-\w+/).first
                    next if ref_group_id.nil?
                    myinstance.subscribe_to_referenced_security_group(ref_group_id)
                  }
          ...
          myinstance.subscribe_to_event("hva.#{myinstance.node.manifest.node_instance_id}/instance_terminated") do |args|
            @worker_thread.pass {
              ...
              inst[:vif].each { |vif|
                vif[:security_groups].each { |secg|
                  unless other_insts_groups.member?(secg)
                    myinstance.unsubscribe_from_security_group(secg)
                    ...
                    myinstance.controller.get_security_groups(secg)[:rules].each { |rule|
                      ...
                      other_groups = myinstance.controller.get_security_groups.delete_if {|group| group[:uuid] == secg}
                      next unless other_groups.find { |group|
                        not group[:rules].find { |rule| rule[:ip_source].scan(/sg-\w+/).first == ref_group_id }.nil?
                      }.nil?
                      
                      myinstance.unsubscribe_from_referenced_security_group(ref_group_id)
                    }
                  end
                }
      ...
      def subscribe_to_local_vnic(vnic_id)
        subscribe_to_event("#{vnic_id}/joined_group") do |args|
          @worker_thread.pass {
            ...
            controller.join_live_security_group(vnic_id,group_id)
        ...
        subscribe_to_event("#{vnic_id}/left_group") do |args|
          @worker_thread.pass {
            ...
            controller.leave_live_security_group(vnic_id,group_id)
            ...
            if controller.get_local_vnics_in_group(group_id).empty?
              unsubscribe_from_security_group(group_id)
              if controller.get_local_groups_that_reference_group(group_id).empty?
      ...
      def subscribe_to_referenced_security_group(ref_group_id)
        subscribe_to_event("#{ref_group_id}/vnic_joined") do |args|
          @worker_thread.pass {
            ...
            controller.join_security_group(vnic_id,ref_group_id)
          }
        end
        ...
        subscribe_to_event("#{ref_group_id}/vnic_left") do |args|
          @worker_thread.pass {
            vnic_id = args[0]
            logger.debug "event caught: #{ref_group_id}/vnic_left"
                        
            # Isolation rules if we have local vnics in this group
            controller.leave_security_group(vnic_id,ref_group_id)
          }
        end
      end

      def subscribe_to_security_group(group_id)
        subscribe_to_referenced_security_group(group_id)
        
        subscribe_to_event("#{group_id}/referencer_added") do |args|
          @worker_thread.pass {
            ref_group_id = args[0]
            #p "event caught: #{group_id}/referencer_added :args => #{args.inspect}"
            logger.debug "event caught: #{group_id}/referencer_added"
            
            controller.add_referencer(group_id,ref_group_id)
        ...
        subscribe_to_event("#{group_id}/referencer_removed") do |args|
          @worker_thread.pass {
            ref_group_id = args[0]
            logger.debug "event caught: #{group_id}/referencer_removed"
            
            controller.remove_referencer(group_id,ref_group_id)
            ...
            if controller.get_instances.find { |inst|
              not inst[:vif].find { |vnic| vnic[:security_groups].member?(ref_group_id) }.nil?
            }.nil? || controller.get_security_groups.find { |group|
              not group[:rules].find { |rule| rule[:ip_source].scan(/sg-\w+/).first == ref_group_id }.nil?
            }.nil?
        ...
        subscribe_to_event("#{group_id}/rules_updated") do |args|
          @worker_thread.pass {
            ...
            # Determine referenced groups before change
            refs_before = controller.get_security_groups(group_id)[:referencees].map { |r|
              r[:uuid]
            }
            
            # Apply the updated security group's rules
            controller.update_security_group(group_id)
            
            # Determine referenced groups after change
            refs_after = controller.get_security_groups(group_id)[:referencees].map { |r|
              r[:uuid]
            }
            
            # Subscribe to new referenced groups
            refs_after.each { |ref_group_id|
              # We might be subscribed to them already but that's ok. In that case nothing will happen.
              subscribe_to_referenced_security_group(ref_group_id) unless refs_before.member?(ref_group_id)
            }
            
            # Unsubscribe from groups that are no longer referenced
            refs_before.each { |ref_group_id|
              # Don't unsubscribe if there are local vnics in the group
              next unless controller.get_instances.find { |inst|
                not inst[:vif].find { |vnic| vnic[:security_groups].member?(ref_group_id) }.nil?
              }.nil?
              
              # Don't unsubscribe if there are other security groups referencing this group
              next unless controller.get_security_groups.find { |group|
                not group[:referencees].find { |r| r[:uuid] == ref_group_id }.nil?
              }.nil?
      ...
      def unsubscribe_from_security_group(group_id)
        # Unsubscribe from vnic_joined and vnic_left if there are not other security groups referencing this one
        unsubscribe_from_referenced_security_group(group_id) if controller.get_security_groups.find { |group|
          not group[:referencees].find { |r| r[:uuid] == group_id }.nil?
        }.nil?
      ...
      def init_netfilter
        begin
          ...
          init_iptables(inst_maps) if Dcmgr.conf.enable_iptables
          init_ebtables(inst_maps, viftable_map) if Dcmgr.conf.enable_ebtables
          init_static_nat(inst_maps) if Dcmgr.conf.enable_iptables && Dcmgr.conf.enable_ebtables
          init_bandwidth_limit(networks = rpc.request('hva-collector', 'get_networks')) if Dcmgr.conf.enable_iptables
          sleep 1
      ...
      def add_netfilter_by_instance_id(inst_id)
        ...
        if Dcmgr.conf.enable_ebtables && Dcmgr.conf.enable_iptables
          cmds << build_nat_chains(inst_map)
          cmds << nat_exceptions(inst_map) unless Dcmgr.conf.use_ipset
      ...
      def delete_netfilter_by_instance_id(inst_id)
        ...
        cmds << build_ebtables_basic_part(vif_map, inst_map, :delete) if Dcmgr.conf.enable_ebtables
        ...
        cmds << build_iptables_basic_part(vif_map, inst_map, :delete) if Dcmgr.conf.enable_iptables
      ...
      def do_exec(cmds)
        ...
        cmds.flatten.uniq.each { |cmd|
          puts cmd if Dcmgr.conf.verbose_netfilter == true
          recmds << cmd
        }
        recmds << "#{eos}"
      ...
      def init_static_nat(inst_maps = [])
        ...
        inst_maps.each { |inst_map|
          ...
          accept_cmds << nat_exceptions(inst_map) unless Dcmgr.conf.use_ipset
      ...
      def build_ebtables_basic_part(vif_map, inst_map, action = :create)
        ...
        if Dcmgr.conf.packet_drop_log
          basic_cmds << "ebtables -#{actions[action]} s_#{vif_map[:uuid]}       --log-level 4 --log-ip --log-arp --log-prefix 'D s_#{vif_map[:uuid]}:'       -j CONTINUE" if Dcmgr.conf.packet_drop_log
          basic_cmds << "ebtables -#{actions[action]} s_#{vif_map[:uuid]}_d_hst --log-level 4 --log-ip --log-arp --log-prefix 'D s_#{vif_map[:uuid]}_d_hst:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
        end
        ...
        basic_cmds << "ebtables -#{actions[action]} s_#{vif_map[:uuid]}_arp       --protocol arp --arp-mac-src ! #{vif_map[:mac]} --log-ip --log-arp --log-prefix 'Dmc s_#{vif_map[:uuid]}_arp:'       -j CONTINUE" if Dcmgr.conf.packet_drop_log
        basic_cmds << "ebtables -#{actions[action]} s_#{vif_map[:uuid]}_d_hst_arp --protocol arp --arp-mac-src ! #{vif_map[:mac]} --log-ip --log-arp --log-prefix 'Dmc s_#{vif_map[:uuid]}_d_hst_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
        ...
        # guest <- * (broadcast)
        basic_cmds << "ebtables -#{actions[action]} d_#{vif_map[:uuid]}_arp       --protocol arp                          --arp-mac-dst 00:00:00:00:00:00 --log-ip --log-arp --log-prefix 'Amc d_#{vif_map[:uuid]}_arp:'     -j CONTINUE" if Dcmgr.conf.packet_drop_log
        basic_cmds << "ebtables -#{actions[action]} d_#{vif_map[:uuid]}_s_hst_arp --protocol arp --arp-ip-src=#{hva_ipv4} --arp-mac-dst 00:00:00:00:00:00 --log-ip --log-arp --log-prefix 'Amc d_#{vif_map[:uuid]}_hst_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
        ...
        # guest <- *
        basic_cmds << "ebtables -#{actions[action]} d_#{vif_map[:uuid]}_arp       --protocol arp --arp-mac-dst ! #{vif_map[:mac]} --log-ip --log-arp --log-prefix 'Dmc d_#{vif_map[:uuid]}_arp:'       -j CONTINUE" if Dcmgr.conf.packet_drop_log
        basic_cmds << "ebtables -#{actions[action]} d_#{vif_map[:uuid]}_s_hst_arp --protocol arp --arp-mac-dst ! #{vif_map[:mac]} --log-ip --log-arp --log-prefix 'Dmc d_#{vif_map[:uuid]}_s_hst_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
        ...
        inst_map[:ips].each { |ipv4|
          # guest -> *
          basic_cmds << "ebtables -#{actions[action]} s_#{vif_map[:uuid]}_arp       --protocol arp --arp-ip-src ! #{ipv4} --log-ip --log-arp --log-prefix 'Dip s_#{vif_map[:uuid]}_arp:'       -j CONTINUE" if Dcmgr.conf.packet_drop_log
          basic_cmds << "ebtables -#{actions[action]} s_#{vif_map[:uuid]}_d_hst_arp --protocol arp --arp-ip-src ! #{ipv4} --log-ip --log-arp --log-prefix 'Dip s_#{vif_map[:uuid]}_d_hst_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
          ...
          # guest <- *
          basic_cmds << "ebtables -#{actions[action]} d_#{vif_map[:uuid]}_arp       --protocol arp --arp-ip-dst ! #{ipv4} --log-ip --log-arp --log-prefix 'Dip d_#{vif_map[:uuid]}_arp:'       -j CONTINUE" if Dcmgr.conf.packet_drop_log
          basic_cmds << "ebtables -#{actions[action]} d_#{vif_map[:uuid]}_s_hst_arp --protocol arp --arp-ip-dst ! #{ipv4} --log-ip --log-arp --log-prefix 'Dip d_#{vif_map[:uuid]}_s_hst_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
      ...
      def build_ebtables_group_part(vif_map, inst_map, viftable_map, action = :create)
        ...
        same_subnet_ipv4s.uniq.reverse_each do |ipv4|
          ...
          if ipv4 == hva_ipv4
            ...
            group_cmds << "ebtables -#{actions[action]} s_#{vif_map[:uuid]}_d_hst_arp --protocol arp --arp-ip-src #{vif_map[:ipv4]} --arp-ip-dst #{ipv4} --log-ip --log-arp --log-prefix 'Afw s_#{vif_map[:uuid]}_d_hst_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
            ...
            # guest->guest
            group_cmds << "ebtables -#{actions[action]} d_#{vif_map[:uuid]}_arp       --protocol arp --arp-ip-src #{ipv4} --arp-ip-dst #{vif_map[:ipv4]} --log-ip --log-arp --log-prefix 'Afw d_#{vif_map[:uuid]}_arp:'       -j CONTINUE" if Dcmgr.conf.packet_drop_log
            ...
            unless viftable_map[ipv4].nil?
              # guest->guest
              group_cmds << "ebtables -#{actions[action]} d_#{viftable_map[ipv4]}_arp       --protocol arp --arp-ip-src #{vif_map[:ipv4]} --arp-ip-dst #{ipv4} --log-ip --log-arp --log-prefix 'Arv d_#{viftable_map[ipv4]}_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
              ...
              # guest->host
              group_cmds << "ebtables -#{actions[action]} s_#{viftable_map[ipv4]}_d_hst_arp --protocol arp --arp-ip-src #{ipv4} --arp-ip-dst #{vif_map[:ipv4]} --log-ip --log-arp --log-prefix 'Arv s_#{viftable_map[ipv4]}_d_hst_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
      ...
      def build_ebtables_final_part(vif_map, action = :create)
        ...
        ################################
        # deny,allow
        final_cmds << "ebtables -#{actions[action]} d_#{vif_map[:uuid]}_arp       --log-level 4 --log-ip --log-arp --log-prefix 'D d_#{vif_map[:uuid]}_arp:'       -j CONTINUE" if Dcmgr.conf.packet_drop_log
        final_cmds << "ebtables -#{actions[action]} s_#{vif_map[:uuid]}_d_hst_arp --log-level 4 --log-ip --log-arp --log-prefix 'D s_#{vif_map[:uuid]}_d_hst_arp:' -j CONTINUE" if Dcmgr.conf.packet_drop_log
        ...
        [ 's', 'd' ].each do |bound|
          protocol_map.each { |k,v|
            ...
            ["#{bound}_#{vif_map[:uuid]}", "#{bound}_#{vif_map[:uuid]}_#{k}"].each { |chain|
              basic_cmds << "iptables -#{actions[action]} #{chain}_drop -j LOG --log-level 4 --log-prefix 'D #{chain}:'" if Dcmgr.conf.packet_drop_log
