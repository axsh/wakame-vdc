    ...
    class ServiceNetfilter < Isono::NodeModules::Base
      ...
      initialize_hook do
        ...
        @worker_thread.pass {
          ...
          (@cache.get_all_local_vnics + @cache.get_all_empty_vnics).each { |vif|
            ...
            @task_manager.apply_vnic_tasks(vif,TaskFactory.create_tasks_for_vnic(vif,@cache.get_vnic_network(vif[:uuid]),friends,security_groups,node))
        ...
        myinstance.subscribe_to_event("hva.#{myinstance.node.manifest.node_instance_id}/vnic_created") do |args|
          @worker_thread.pass {
            ...
            @task_manager.apply_vnic_tasks(vnic,TaskFactory.create_tasks_for_vnic(vnic,@cache.get_vnic_network(vnic[:uuid]),friends,security_groups,node))
        ...
        myinstance.subscribe_to_event("hva.#{myinstance.node.manifest.node_instance_id}/vnic_destroyed") do |args|
          @worker_thread.pass {
            ...
            if @cache.vnic_has_ip?(vnic_id)
              ...
              @task_manager.remove_vnic_tasks(vif, TaskFactory.create_nat_tasks_for_vnic(vif,network,self.node) )
            end
        ...
        subscribe_to_event("#{ref_group_id}/vnic_joined") do |args|
          @worker_thread.pass {
            ...
            @cache.get_all_security_groups.each { |secg_map|
              ...
              lvnics.each { |lvnic|
                @task_manager.apply_vnic_tasks(lvnic,TaskFactory.create_tasks_for_ARP_isolation(lvnic,[referencer_vnic_map],node))
              }
            }
      ...
      def handle_ref_group_vnic(group_id,vnic_id,action = :apply)
        ...
        @cache.get_all_security_groups.each { |secg_map|
          ...
          vnics = @cache.get_local_vnics_in_group(group_id)
