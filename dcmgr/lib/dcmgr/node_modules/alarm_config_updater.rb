...
module Dcmgr::NodeModules
  ...
  class AlarmConfigUpdater < Isono::NodeModules::Base
    ...
    def write_config
      ...
      @alarms.each do |a|
        alm << {
          ...
          :notification_periods => a[:notification_periods],
          :alarm_action => format_notification_action(
                              a[:alarm_actions]['notification_id'],
                              a[:alarm_actions]['notification_message_type']),
          :insufficient_data_action => format_notification_action(
                                          a[:insufficient_data_actions]['notification_id'],
                                          a[:insufficient_data_actions]['notification_message_type'])
        }
      end
    ...
    def format_notification_action(id, message_type)
      "#{id}:#{message_type}"
    end
