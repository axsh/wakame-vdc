# -*- coding: utf-8 -*-
require 'isono'
require 'dcmgr/constants/instance'
require 'dcmgr/constants/volume'
    ...
    class Scheduler < Isono::NodeModules::Base
      ...
      def evacuate_from(host_node_id, keep_node_offline=false)
        ...
        begin
          ...
          src_host_node.scheduling_enabled = false
          ...
          if keep_node_offline == false
            src_host_node.scheduling_enabled = true
            src_host_node.save_changes
          end
        end


        logger.info("Scheduling result for the evacuation process from #{src_host_node.canonical_uuid}")
        logger.info("Total number of instances on the host node: #{running_instances_ds.count.to_i}")
        if scheduled_instances.size > 0
          logger.info("Scheduled instance(s): #{scheduled_instances.size}")
          scheduled_instances.each { |i|
            logger.info("  #{i.canonical_uuid} -> #{i.host_node.canonical_uuid}")
          }
        end
        if failed_instances.size > 0
          logger.info("Scheduling failed instance(s): #{failed_instances.size}")
          logger.info( failed_instances.map{|i| i.canonical_uuid } )
        end
        if skipped_instances.size > 0
          logger.info("Skipped instance(s): #{skipped_instances.size}")
          logger.info( skipped_instances.map{|i| i.canonical_uuid } )
        end
      ...
      def apply_ha_scheduler(instance)
        state_transition = []
        case instance.state
        when C::Instance::STATE_HALTED, C::Instance::STATE_STOPPING
          state_transition = [instance.state, C::Instance::STATE_MIGRATING]
          instance.state = Constants::Instance::STATE_MIGRATING
          db.after_commit do
            self.job.submit("migration-handle.#{instance.host_node.node_id}",
                            'initialize_halted_instance', instance.canonical_uuid)
          end
        when C::Instance::STATE_RUNNING, C::Instance::STATE_STARTING
          state_transition = [instance.state, C::Instance::STATE_PENDING]
          # Push back instance state to pending from running.
          instance.state = Constants::Instance::STATE_PENDING
        ...
        when C::Instance::STATE_SHUTTING_DOWN
          # TODO: finalize instance.
          # TODO: also need to volumes to clean.
        else
          # TODO: raise error and skip scheduling
          raise "Unsupported instance state to schedule for HA: #{instance.canonical_uuid}, #{instance.state}"
        end
        instance.save_changes
        ...
        logger.info("#{instance.canonical_uuid} is located at #{instance.host_node.canonical_uuid}" +
                    " #{state_transition[0]} -> #{state_transition[1]}")
