  ...
  module NodeModules
    class Scheduler < Isono::NodeModules::Base
      ...
      def schedule_instance(instance_id)
        ...
        #TODO: Do something about the lack of security groups in the vifs parameter when coming from the gui
        instance.network_vif.each {|vif|
          svc_type.mac_address.schedule(vif) if  vif.mac_addr.nil? || vif.mac_addr.empty?
          #TODO: what about nat lease?
          svc_type.ip_address.schedule(vif) if vif.direct_ip_lease.empty?
          vif.save_changes
        }
        instance.save
        ...
        vif_params.each { |name, param|
          ...
          if param['monitors'].is_a?(Hash)
            param['monitors'].each { |idx, m|
              vif.add_network_vif_monitor({'params'=>''}.merge(m))
            }
