  ...
  class ServiceNatbox < Isono::NodeModules::Base
    ...
    def network_route_created(header, body)
      body.each do |network_route_id|
        network_route = get_network_route(network_route_id)
    ...
    def network_route_deleted(header, body)
      body.each do |network_route_id|
        network_route = get_network_route(network_route_id)
        remove_nat(network_route[:outer_ipv4], network_route[:inner_ipv4])
      end
    end
    ...
    def restore
      remove_all_nat
      # TODO paginate
      rpc.request('natbox-collector', 'get_alive_network_routes').each do |network_route|
        add_nat(network_route[:outer_ipv4], network_route[:inner_ipv4])
      end
    end
    ...
    def ovs_ofctl
      ...
      @ovs_ofctl = Dcmgr::VNet::OpenFlow::OvsOfctl.new.tap do |ovs_ofctl|
        ovs_ofctl.switch_name = Dcmgr.conf.dc_networks[:external].bridge
      end
    end
