    ...
    class MetricMonitor < Isono::NodeModules::Base
      ...
      def add_metric_timer(alarm)
        EM.add_periodic_timer(alarm[:evaluation_periods]) do
          ...
          logger.debug("Evaluate metric data: #{alarm[:metric_name]} #{evaluation_time}")
          if @evaluate.evaluate(alarm, evaluation_time)
            logger.debug(alarm)
          end
          @evaluate.delete(resource_id, alarm[:metric_name], evaluation_time)
        end            
      end
      ...
      def parse_pidstat(data, metric_name)
        ...
        case metric_name
        when "cpu"
          initial_keys = ['time', 'pid', 'usr_usage', 'system_usage', 'guest_usage', 'usage', 'cpu_number', 'cswch', 'nvcswch', 'usr_ms', 'system_ms', 'guest_ms']
        when "memory"
