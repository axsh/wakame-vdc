  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    ...
    def add_eth_now switch, port
      ...
      networks_map.each { |network_map|
        ...
        if network.services.has_key?(:metadata) and network.services[:metadata].ip.to_s != Isono::Util.default_gw_ipaddr.to_s
    ...
    def create_network network_map
      ...
      if !network.virtual
        ...
        if network_map[:metadata_server] && network_map[:metadata_server_port]
          if network_map[:metadata_server].to_s == Isono::Util.default_gw_ipaddr.to_s
            network.add_service(switch[1], {
                                  :name => 'metadata',
