...
#        nw = M::Network["nw-demonet"] || UnknownUUIDError.raise("nw-demonet")
    ...
    class CustomEventHandler < Trema::EventHandler
      ...
      def init_event_handler
        logger.info "Called init_event_handler."
      end

      def finalize_event_handler
        logger.info "Called finalize_event_handler."
      end

      def start_event_handler
        logger.info "Called start_event_handler."

        # EM.run {
        #   puts "Trema thread EM.run.\n"
        # }

        logger.info "CustomEventHandler exiting start_event_handler."
      end

      def stop_event_handler
        logger.info "Called stop_event_handler."
        EM.stop
      end

      def run_event_handler_once
        logger.info "Called run_event_handler_once."
      end
      
      def add_fd_event fd, connection
        logger.info "Called immediate add_fd_event, #{fd}."
        
        $fd_set[ fd ] = EM.watch fd, EventFdConnection, connection
      end
      
      def delete_fd_event fd
        logger.info "Called delete_fd_event, #{fd}."
      ...
      def notify_readable_event fd, state
        logger.debug "Called notify_readable_event, #{fd}."

        if fd < 0
          logger.error "Called notify_readable_event with negative fd: #{fd}, #{state}."
          return
        end
      ...
      def notify_writable_event fd, state
        logger.debug "Called notify_writable_event, #{fd}."

        if fd < 0
          logger.error "Called notify_writable_event with negative fd: #{fd}, #{state}."
          return
        end
      ...
      def is_notifying_readable_event fd
        logger.debug "Called is_notifying_readable_event, #{fd}."
      end

      def is_notifying_writable_event fd
        logger.debug "Called is_notifying_writable_event, #{fd}."
      end
    end
