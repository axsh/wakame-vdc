  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    ...
    def create_network network_map
      ...
      if not network.ipv4_gw.nil?
        ...
        if !network.metadata_server_ip.nil? and !network.metadata_server_port.nil?
          if network.metadata_server_ip.to_s == Isono::Util.default_gw_ipaddr.to_s
            network.install_metadata_server Dcmgr::VNet::OpenFlow::OpenFlowController::OFPP_LOCAL, @openflow_controller.local_hw
          else
            switch[1].packet_handlers <<
              Dcmgr::VNet::OpenFlow::PacketHandler.new(Proc.new { |switch,port,message|
                                                         port.network.metadata_server_output.nil? and
                                                         message.arp? and
                                                         message.arp_oper == Racket::L3::ARP::ARPOP_REPLY and
                                                         message.arp_spa.to_s == port.network.metadata_server_ip.to_s and
                                                         message.arp_tpa.to_s == Isono::Util.default_gw_ipaddr.to_s
                                                       }, Proc.new { |switch,port,message|
                                                         port.network.install_metadata_server(message.in_port, message.arp_sha)
                                                       })
          end
          ...
          switch[1].packet_handlers <<
            Dcmgr::VNet::OpenFlow::PacketHandler.new(Proc.new { |switch,port,message|
                                                       ...
                                                       if port.network.metadata_server_ip.to_s == Isono::Util.default_gw_ipaddr.to_s
                                                         switch.install_dnat_entry(message, TABLE_METADATA_OUTGOING, TABLE_METADATA_INCOMING,
                                                                                   port.network.metadata_server_output,
                                                                                   port.network.local_hw,
                                                                                   port.network.metadata_server_ip.to_s,
                                                                                   port.network.metadata_server_port)
                                                       else
                                                         switch.install_dnat_entry(message, TABLE_METADATA_OUTGOING, TABLE_METADATA_INCOMING,
                                                                                   port.network.metadata_server_output,
                                                                                   port.network.metadata_server_mac,
                                                                                   port.network.metadata_server_ip.to_s,
                                                                                   port.network.metadata_server_port)
                                                       end
