  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    ...
    def create_network network_map
      ...
      if network.services[:dhcp].nil?
        network.add_service(switch[1], {
                              :name => 'dhcp',
                              :mac_addr => openflow_controller.local_hw,
                              :address => (network_map[:dhcp_server] and !network_map[:dhcp_server].empty? ?
                                           network_map[:dhcp_server] :
                                           (IPAddr.new(network_map[:ipv4_network]) | IPAddr.new("0.0.0.1")).to_s)
                            })
      end
      ...
      if network.services[:dns].nil?
        network.add_service(switch[1], {
                              :name => 'dns',
                              :mac_addr => openflow_controller.local_hw,
                              :address => (network_map[:dns_server] and !network_map[:dns_server].empty? ?
                                           network_map[:dns_server] :
                                           nil)
                            })
      end
