...
module Dcmgr
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_instance_now port
        ...
        if network_id == 1
          ...
          if openflow_controller.networks[network_id].nil?
            ...
            dhcp_ip = IPAddr.new(ip_map[:network][:ipv4_network]) | IPAddr.new("0.0.0.1")

            openflow_controller.install_virtual_network(network_id, openflow_controller.local_hw, dhcp_ip)
            openflow_controller.networks[network_id].ipv4_network = IPAddr.new(ip_map[:network][:ipv4_network])
            openflow_controller.networks[network_id].prefix = ip_map[:network][:prefix]
          end

          network = openflow_controller.networks[network_id]
          network.virtual = true

          openflow_controller.install_virtual_route network.id, port, mac_lease, ip_lease

          # openflow_controller.install_virtual_arp_route network.id, port, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, openflow_controller.local_hw.to_s, ip_map[:network][:dhcp_server]
          # openflow_controller.install_virtual_arp_route network.id, port, mac_lease, ip_lease, openflow_controller.eth_number, "88:43:e1:42:4f:60", "192.168.2.1"

          # openflow_controller.install_virtual_static_d_transport network.id, 17, port, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, ip_map[:network][:dhcp_server], 67
          # openflow_controller.install_virtual_static_d_transport network.id, 17, port, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, ip_map[:network][:dhcp_server], 68
        end
      ...
      def add_tunnel port
        # FIXME: Hack to ensure all networks are loaded...
        # worker_thread.pass { add_tunnel_now port }
        worker_thread.pass { EventMachine::Timer.new(5) { add_tunnel_now port } }
      end
      ...
      def add_tunnel_now port
        ...
        port.lock.synchronize {
          ...
          openflow_controller.networks.each { |id, network|
            # if network.virtual
            if network.id == 2
              logger.info "adding network to tunnel: id:#{network.id}."

              network.add_port port.port_info.number
              openflow_controller.update_network network.id
