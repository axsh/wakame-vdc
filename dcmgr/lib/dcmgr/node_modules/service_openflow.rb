    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def create_network network_map
        ...
        if not network.ipv4_gw.nil?
            ...
            worker_thread.pass {
              ...
              if body[:host_ip] == Isono::Util.default_gw_ipaddr.to_s
                ...
                if body[:send_hosts] == true
                  event.publish("ovs/network_added/#{network_id}", :args=>{:network_id => network_map[:id], :send_hosts => false, :host_id => @node.node_id, :host_ip => Isono::Util.default_gw_ipaddr.to_s})
                end
              end
            }
          end
          ...
          event.publish("ovs/network_added/#{network_id}", :args=>{:network_id => network_map[:id], :send_hosts => true, :host_id => @node.node_id, :host_ip => Isono::Util.default_gw_ipaddr.to_s})
        end
      ...
      # def debug
      #   @debug ||= Dcmgr::VNet::OpenFlow::Debug.new(@node)
      # end
