...
#          logger.info "Found port: port:#{each.number} mac:#{each.hw_addr.to_s} name:#{each.name} config:#{each.config} state:#{each.state} curr:#{each.curr} advertised:#{each.advertised} supported:#{each.supported} peer:#{each.peer}."
      ...
      def packet_in datapath_id, message
        ...
        if tmp_msg.dl_type == 0x0806
          if arp_translate_incoming_map.has_key? tmp_msg.nw_dst
            logger.info "Rewriting incoming ARP packet: #{tmp_msg.nw_dst} => #{arp_translate_incoming_map[tmp_msg.nw_dst]}."
            # TODO: 
          end

          if arp_translate_incoming_map.has_key? tmp_msg.nw_dst
            logger.info "Rewriting outcoming ARP packet: #{tmp_msg.nw_dst} => #{arp_translate_incoming_map[tmp_msg.nw_dst]}."
            # TODO: 
          end
      ...
      def install_snat_entry datapath_id, inside_port, inside_hw, inside_ip, outside_port, outside_hw, outside_ip
        ...
        send_flow_mod_add(datapath_id,
                          ...
                          :priority => 0x100,
                          :actions => [ # Trema::ActionSetDlDst.new( Trema::Mac.new(inside_hw) ),
                                        Trema::ActionSetNwDst.new( Trema::IP.new(inside_ip) ),
                                        Trema::ActionOutput.new( OFPP_CONTROLLER ) ] )
        send_flow_mod_add(datapath_id,
                          ...
                          Trema::Match.new( :in_port => inside_port, :dl_type => 0x0806,
                                            :dl_src => inside_hw, :nw_src => inside_ip ),
                          :priority => 0x100,
                          :actions => [ Trema::ActionSetDlSrc.new( Trema::Mac.new(outside_hw) ),
                                        Trema::ActionSetNwSrc.new( Trema::IP.new(outside_ip) ),
                                        Trema::ActionOutput.new( OFPP_CONTROLLER ) ] )

        # Hack that is not really safe.
        arp_translate_incoming_map[outside_ip] = { :nw_dst => inside_ip, :output => inside_port }
        arp_translate_outcoming_map[inside_ip] = { :nw_src => outside_ip, :output => OFPP_NORMAL }
        ...
        send_flow_mod_add(datapath_id,
                          ...
                          :actions => [ Trema::ActionSetDlDst.new( Trema::Mac.new(inside_hw) ),
                                        Trema::ActionSetNwDst.new( Trema::IP.new(inside_ip) ),
                                        Trema::ActionOutput.new(inside_port) ] )
        ...
        send_flow_mod_add(datapath_id,
                          ...
                          :actions => [ Trema::ActionSetDlSrc.new( Trema::Mac.new(outside_hw) ),
                                        Trema::ActionSetNwSrc.new( Trema::IP.new(outside_ip) ),
                                        Trema::ActionOutput.new( dest_outside_port ) ] )
      end
    end
