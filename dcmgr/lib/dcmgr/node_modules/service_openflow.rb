  ...
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_openflow_by_instance_id(inst_id)
        logger.info "add_openflow_by_instance_id: inst_id:#{inst_id}"
      ...
      def delete_openflow_by_instance_id(inst_id)
        logger.info "delete_openflow_by_instance_id: inst_id:#{inst_id}"
      ...
      def add_instance_now port
        ...
        vif_map = inst_map[:vif].find(:vif_id => port.port_info.name).next
        ...
        openflow_controller.install_static_d_transport 17, port_number, mac_lease, ip_lease, ip_map[:network][:dns_server], 53
        ...
        # Testing virtual network.
        # openflow_controller.install_virtual_route 1, port_number, mac_lease, ip_lease
        # openflow_controller.install_virtual_arp_route 1, port_number, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, openflow_controller.local_hw.to_s, ip_map[:network][:dhcp_server]

        # openflow_controller.install_virtual_arp_route 1, port_number, mac_lease, ip_lease, openflow_controller.eth_number, "88:43:e1:42:4f:60", "192.168.2.1"

        # openflow_controller.install_virtual_static_d_transport 1, 17, port_number, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, ip_map[:network][:dhcp_server], 67
        # openflow_controller.install_virtual_static_d_transport 1, 17, port_number, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, ip_map[:network][:dhcp_server], 68
    ...
    class CustomEventHandler < Trema::EventHandler
      ...
      def init_timer
        logger.info "Called CustomEventHandler init_timer."
      end

      def finalize_timer
        logger.info "Called CustomEventHandler finalize_timer."
      end

      def add_timer_event_callback timer
        logger.info "Called CustomEventHandler: first:#{timer.inspect} interval:#{timer.interval} expiration:#{timer.expiration}."

        timer.handle.cancel if not timer.handle.nil?

        timer.handle = EventMachine::Timer.new(timer.interval) do
          logger.info "Calling timer event: first:#{timer.inspect} interval:#{timer.interval} expiration:#{timer.expiration}."
          timer.call
        end
      end

      def add_periodic_event_callback interval, timer
        logger.info "Called CustomEventHandler: timer:#{timer.inspect} interval:#{interval}."

        timer.handle.cancel if not timer.handle.nil?

        timer.handle = EventMachine::PeriodicTimer.new(interval) do
          logger.info "Calling periodic timer event: timer:#{timer.inspect} interval:#{interval}."
          timer.call
        end
      end

      def delete_timer_event timer
        logger.info "Called CustomEventHandler: timer:#{timer.inspect}."
      ...
      def execute_timer_events
        logger.info "Called CustomEventHandler execute_timer_events."
      end
