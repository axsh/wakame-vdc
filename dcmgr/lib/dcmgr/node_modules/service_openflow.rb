  ...
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_instance_now port
        ...
        logger.info "vif_map: #{vif_map.inspect}"
        logger.info "ip_map: #{ip_map.inspect}"
        ...
        if not ip_lease =~ /^10\.10[01]\./
          network = openflow_controller.networks[0]
          ...
          network = openflow_controller.networks[1] if ip_lease =~ /^10\.100\./
          network = openflow_controller.networks[2] if ip_lease =~ /^10\.101\./
