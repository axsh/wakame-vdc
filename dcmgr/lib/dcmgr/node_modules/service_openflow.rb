...
module Dcmgr::NodeModules
  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    OF = Dcmgr::VNet::OpenFlow

    include Dcmgr::Logger
    include Dcmgr::Helpers::NicHelper
    include OF::OpenFlowConstants
    ...
    initialize_hook do
      ...
      myinstance.worker_thread.pass {
        ...
        myinstance.event.subscribe("vnet/network_node_added", '#') { |header, body| myinstance.event_vnet_network_node_added(header, body) }
        myinstance.event.subscribe("vnet/network_services",   '#') { |header, body| myinstance.event_vnet_network_services(header, body) }
      }
    end
    ...
    def add_eth switch, port
      ...
      if OF::NetworkPhysical.add_eth_port(port.datapath.datapath_id, port.port_info.number)
        port.datapath.add_flows(OF::NetworkPhysical.physical_flood_flows(port.datapath.datapath_id))
      end
      ...
      worker_thread.pass {
        ...
        networks_map.each { |network_map|
          ...
          metadata_service = network.get_service(:metadata)

          if metadata_service and metadata_service.ip.to_s != switch.bridge_ipv4
            metadata_service.request_mac(switch, port)
          end
        }
      }
    end
    ...
    def add_instance_now switch, port
      ...
      raise ArgumentError, "Unknown Nic: #{port.port_info.name}." unless vnic_map
      logger.debug "vnic_map: #{vnic_map.inspect}"

      if vnic_map[:network_id].nil?
        logger.debug "vif not attached to network, skipping."

        # Remove port from bridge as it was probably added by openvz
        # to the default bridge.
        switch.datapath.ovs_ofctl.del_port(port.port_info.name)
        return
      end

      ip_lease = vnic_map[:address]
      raise ArgumentError, "Unknown IPv4: #{port.port_info.name}." unless ip_lease
      ...
      port.lock.synchronize {
        ...
        if !network.virtual
          ...
          network.datapath.add_flows(network.class.physical_flood_flows(network.datapath.datapath_id))
        end

        network.add_port port_number, true
        network.update

        vnic_map[:network_routes  ].each { |route  | network.update_route_port(switch, route, port) }
        vnic_map[:network_services].each { |service| network.update_service_port(switch, service, port) }
    ...
    def add_patch switch, port
      worker_thread.pass { add_patch_now switch, port }
    end

    def add_patch_now switch, port
      logger.info "Got bridge-to-bridge patch port: name:#{port.port_info.name}."
      ...
      # Note that vnet_id may be different from the actual GRE
      # patch id used.
      bridge_self = port.port_info.name[/^patch-([^-]*)-[^-]*$/, 1]
      bridge_dest = port.port_info.name[/^patch-[^-]*-([^-]*)$/, 1]

      raise "Bridge-to-bridge patch interface name must match 'patch-[^-]*-[^-]*'." unless bridge_self && bridge_dest
    ...
    def delete_patch port
      worker_thread.pass { delete_patch_now port }
    end

    def delete_patch_now port
    end
    ...
    def create_network network_map
      ...
      if network_map[:network_mode] == "l2overlay"
        network = OF::NetworkVirtual.new(switch[1].datapath, network_id)
      else
        network = OF::NetworkPhysical.new(switch[1].datapath, network_id)
      end
      ...
      if !network.virtual
        ...
        network.add_port(OF::OpenFlowController::OFPP_LOCAL, true)

        if network.find_services(:gateway).empty? and
        ...
        if network.find_services(:dhcp).empty?
          network.add_service(switch[1], {
                                :name => 'dhcp',
        ...
        if network.find_services(:dns).empty? and
            (network_map[:dns_server] and !network_map[:dns_server].empty?)
          network.add_service(switch[1], {
                                :name => 'dns',
                                :mac_addr => network.local_hw,
          ...
          if network_map[:domain_name] and !network_map[:domain_name].empty?
            network.get_service(:dns).domain_name = network_map[:domain_name]
          else
            network.get_service(:dns).domain_name = 'vdc.local'
          end
        end
        ...
        if network_map[:metadata_server] && network_map[:metadata_server_port]
          if network_map[:metadata_server].to_s == switch[1].bridge_ipv4
            network.add_service(switch[1], {
                                  :name => 'metadata',
                                  :port => OF::OpenFlowController::OFPP_LOCAL,
                                  :mac_addr => network.local_hw,
        ...
        if network.get_service(:dns) and
            network_map[:domain_name] and !network_map[:domain_name].empty?
          network.get_service(:dns).domain_name = network_map[:domain_name]
        end
        ...
        event.publish("vnet/network_node_added", :args=>{
                        :network_id => network_map[:id],
                        :send_hosts => true,
    ...
    def destroy_network network, remove_flows
      worker_thread.pass {
        logger.info "Removing network: id:#{network.id} remove_flows:#{remove_flows}."
    ...
    def event_vnet_network_node_added(header, body)
      worker_thread.pass {
        next if body[:host_id] == @node.node_id.to_s

        switch, network = openflow_controller.find_network_id(body[:network_id])
        next if switch.nil? or network.nil?

        logger.info("Network added event: network_id:#{network.id} header:#{header.inspect} body:#{body.inspect}.")
        ...
        if body[:send_hosts] == true
          event.publish("ovs/network_node_added", :args=>{
                          :network_id => network_map[:id],
                          :send_hosts => false,
    ...
    def event_vnet_network_services(header, body)
      worker_thread.pass {
        switch, network = openflow_controller.find_network_id(body[:service][:network_id])
        next if switch.nil? or network.nil?

        logger.debug "vnet/network_services: header:#{header.inspect}: body:#{body.inspect}"

        case body[:status].to_sym
        when :add    then network.add_service(switch, body[:service])
        when :delete then network.delete_service(switch, body[:service])
        end
      }
    end
    ...
    def openflow_controller
      @openflow_controller ||= OF::OpenFlowController.new(self)
    end
