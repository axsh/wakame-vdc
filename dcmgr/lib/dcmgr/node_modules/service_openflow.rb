...
module Dcmgr
  module NodeModules
    ...
    class CustomEventHandler < Trema::EventHandler
      ...
      def add_timer_event_callback first, second
        logger.info "Called CustomEventHandler: first:#{first.inspect} second:#{second.inspect}, #{second.period}."

        second.handle.cancel if not second.handle.nil?

        second.handle = EventMachine::Timer.new(second.period) do
          logger.info "Calling timer event."
          second.call
        end
      end

      def add_periodic_event_callback interval, timer
        logger.info "Called CustomEventHandler: interval:#{interval} second:#{timer.inspect}, #{timer.period}."

        timer.handle.cancel if not timer.handle.nil?

        timer.handle = EventMachine::PeriodicTimer.new(interval) do
          logger.info "Calling periodic timer event."
          timer.call
        end
      end

      def delete_timer_event first
        logger.info "Called CustomEventHandler: first:#{first.inspect}, first.handle:#{first.handle.inspect}."
