  ...
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_openflow_by_instance_id(inst_id)
        ...
        add_instance_now port[1].port_info if not port.nil?
      end
      ...
      # Always call in worker thread.
      #
      # TODO: Consider just passing OpenFlowPort.
      def add_instance port_info
        worker_thread.pass { add_instance_now port_info }
      end

      def add_instance_now port_info
        raise "UnknownOpenflowPort" if not openflow_controller.ports.has_key? port_info.number

        nic_map = rpc.request('hva-collector', 'get_nic', port_info.name)
        ip_lease = rpc.request('hva-collector', 'get_iplease_for_nic', port_info.name)
        mac_lease = clean_mac(nic_map[:mac_addr])

        raise ArgumentError, "Unknown Nic: #{port_info.name}" if nic_map.nil?
        raise ArgumentError, "Unknown Nic: #{port_info.name}" if ip_lease.nil?

        openflow_controller.ports[port_info.number].ip = ip_lease
        openflow_controller.ports[port_info.number].mac = mac_lease
        openflow_controller.ports[port_info.number].has_instance = true

        logger.info "adding instance #{port_info.name}."
        logger.info "port: #{port_info.number}"
        logger.info "mac: #{port_info.hw_addr.to_s} <=> #{mac_lease}"
        ...
        openflow_controller.install_route         port_info.number, mac_lease, ip_lease
        openflow_controller.install_arp_antispoof port_info.number, mac_lease, ip_lease
      end
      ...
      def delete_instance_now port
        raise "UnknownOpenflowPort" if not openflow_controller.ports.has_key? port.port_info.number
        return if not openflow_controller.ports[port.port_info.number].has_instance

        ip_lease = openflow_controller.ports[port.port_info.number].ip
        mac_lease = openflow_controller.ports[port.port_info.number].mac

        openflow_controller.ports[port.port_info.number].ip = nil
        openflow_controller.ports[port.port_info.number].mac = nil
        openflow_controller.ports[port.port_info.number].has_instance = nil
