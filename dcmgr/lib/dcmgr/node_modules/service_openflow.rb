  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    ...
    def add_eth_now switch, port
      ...
      if network.services.has_key?(:metadata_server) and network.services[:metadata_server].ip.to_s != Isono::Util.default_gw_ipaddr.to_s
        network.services[:metadata_server].request_mac(switch, port)
      end
    end
      ...
      if !network_map[:physical_network][:interface].nil?
        ...
        if network.services.has_key?(:metadata_server)
          if network.services[:metadata_server].ip.to_s == Isono::Util.default_gw_ipaddr.to_s
            network.services[:metadata_server].install(Dcmgr::VNet::OpenFlow::OpenFlowController::OFPP_LOCAL, @openflow_controller.local_hw)
          end
        end
