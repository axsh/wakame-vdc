  ...
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_openflow_by_instance_id(inst_id)
        ...
        logger.info "Add OpenFlow instance: inst_id:#{inst_id} uuid:#{vif_map[:uuid]} mac:#{vif_map[:mac]} ipv4:#{vif_map[:ipv4]}."
      ...
      def add_openflow_by_nic(nic_uuid)
        raise ArgumentError, "Unknown Nic: #{inst_id}" if nic_uuid.nil?
        nic_map = rpc.request('hva-collector', 'get_nic', nic_uuid)
        raise ArgumentError, "Unknown Nic: #{inst_id}" if nic_map.nil?
        
        logger.info "Add OpenFlow nic: #{nic_map}."

        add_openflow_by_instance_id nic_map[:instance_id]
      end
