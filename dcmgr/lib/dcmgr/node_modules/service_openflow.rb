  ...
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_instance_now port
        ...
        # Testing guest -> *
        openflow_controller.install_icmp               port.port_info.number, mac_lease, ip_lease
        openflow_controller.install_transport_port 6,  port.port_info.number, mac_lease, ip_lease
        openflow_controller.install_transport_port 17, port.port_info.number, mac_lease, ip_lease
      end
      ...
      def add_security_group inst_id, port
        ...
        build_rule(rules).each do |rule|
          ...
          when 'tcp', 'udp'
            if rule[:ip_fport] == rule[:ip_tport]
              logger.info "fake iptables -A d_#{vif_map[:uuid]}_#{rule[:ip_protocol]} -p #{rule[:ip_protocol]} -s #{rule[:ip_source]} --dport #{rule[:ip_fport]} -j ACCEPT"
            else
              logger.info "fake iptables -A d_#{vif_map[:uuid]}_#{rule[:ip_protocol]} -p #{rule[:ip_protocol]} -s #{rule[:ip_source]} --dport #{rule[:ip_fport]}:#{rule[:ip_tport]} -j ACCEPT"
            end
          when 'icmp'
            # icmp
            ...
            if rule[:icmp_type] == -1 && rule[:icmp_code] == -1
              logger.info "fake iptables -A d_#{vif_map[:uuid]}_#{rule[:ip_protocol]} -p #{rule[:ip_protocol]} -s #{rule[:ip_source]} -j ACCEPT"
            else
              logger.info "fake iptables -A d_#{vif_map[:uuid]}_#{rule[:ip_protocol]} -p #{rule[:ip_protocol]} -s #{rule[:ip_source]} --icmp-type #{rule[:icmp_type]}/#{rule[:icmp_code]} -j ACCEPT"
            end
          end
        end
      end
