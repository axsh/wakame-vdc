  ...
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_openflow_by_instance_id(inst_id)
        ...
        add_instance port[1].port_info if not port.nil?
      end
      ...
      def delete_openflow_by_instance_id(inst_id)
        # port = openflow_controller.ports.detect { |f| f[1].owner == inst_id }

        # raise "UnknownOpenflowPort" if port[1].nil?
        # raise "UnknownOpenflowIp" if port[1].ips.first.nil?

        # vif_map = {
        #   :uuid  => port[1].port_info.name
        #   :mac   => port[1].port_info.hw_addr.to_s
        #   :ipv4  => port[1].ips.first
        #   :port  => port[0]
        # }
