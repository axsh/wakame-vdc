  ...
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_instance_now port
        ...
        if not ip_map[:network][:ipv4_gw].nil?
      ...
      def add_tunnel_now port
        ...
        port.lock.synchronize {
          ...
          if not openflow_controller.networks.has_key? vnet_id
            logger.info "Virtual network not yet initialized: network_id:#{vnet_id}."
            EventMachine::Timer.new(5) { add_tunnel_now port } #???
            return
          end

          network = openflow_controller.networks[vnet_id]
