  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    ...
    def create_network network_map
      ...
      network.services[:dhcp] =
        Dcmgr::VNet::OpenFlow::ServiceDhcp.new({ :switch => switch[1],
                                                 :mac => openflow_controller.local_hw,
                                                 :ip => network_map.has_key?(:dhcp_server) ? IPAddr.new(network_map[:dhcp_server]) : nil})
      network.services[:dns] =
        Dcmgr::VNet::OpenFlow::ServiceDns.new({ :switch => switch[1],
                                                :mac => openflow_controller.local_hw,
                                                :ip => network_map.has_key?(:dns_server) ? IPAddr.new(network_map[:dns_server]) : nil})

      if network.services[:dhcp].ip.nil? or network.services[:dhcp].ip.empty?
        network.services[:dhcp].ip = IPAddr.new(network_map[:ipv4_network]) | IPAddr.new("0.0.0.1") 
      end

      if network.services[:dns].ip.nil? or network.services[:dns].ip.empty?
