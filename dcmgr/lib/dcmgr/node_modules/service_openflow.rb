  ...
  module NodeModules
    ...
    class ServiceOpenFlow < Isono::NodeModules::Base
      ...
      def add_instance_now port
        ...
        if network_id == 1
          network = openflow_controller.networks[1]

          openflow_controller.install_route         port_number, mac_lease, ip_lease
          openflow_controller.install_arp_antispoof port_number, mac_lease, ip_lease

          openflow_controller.install_static_d_transport 17, port_number, mac_lease, ip_lease, ip_map[:network][:dns_server], 53 if not ip_map[:network][:dns_server].nil?

          if not ip_map[:network][:dns_server].nil?
            openflow_controller.install_static_d_transport 17, port_number, mac_lease, ip_lease, ip_map[:network][:dhcp_server], 67
            openflow_controller.install_static_d_transport 17, port_number, mac_lease, ip_lease, ip_map[:network][:dhcp_server], 68
          else
            openflow_controller.install_static_d_transport 17, port_number, mac_lease, ip_lease, "0.0.0.0/0", 67
            openflow_controller.install_static_d_transport 17, port_number, mac_lease, ip_lease, "0.0.0.0/0", 68
          end

          add_security_group port, inst_map[:uuid], vif_map

          # Testing guest -> *
          openflow_controller.install_local_icmp          port_number, mac_lease, ip_lease
          openflow_controller.install_local_transport 6,  port_number, mac_lease, ip_lease
          openflow_controller.install_local_transport 17, port_number, mac_lease, ip_lease

        else
          if openflow_controller.networks[network_id].nil?
            raise "No valid ipv4 network defined." if ip_map[:network][:ipv4_network].nil? or not ip_map[:network][:ipv4_network] =~ /\.0$/

            dhcp_ip = ip_map[:network][:ipv4_network].dup
            dhcp_ip[/\.0$/] = ".1"

            openflow_controller.install_virtual_network(network_id, openflow_controller.local_hw, dhcp_ip)
          end

          network = openflow_controller.networks[network_id]

          openflow_controller.install_virtual_route network.id, port_number, mac_lease, ip_lease

          # openflow_controller.install_virtual_arp_route network.id, port_number, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, openflow_controller.local_hw.to_s, ip_map[:network][:dhcp_server]
          # openflow_controller.install_virtual_arp_route network.id, port_number, mac_lease, ip_lease, openflow_controller.eth_number, "88:43:e1:42:4f:60", "192.168.2.1"

          # openflow_controller.install_virtual_static_d_transport network.id, 17, port_number, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, ip_map[:network][:dhcp_server], 67
          # openflow_controller.install_virtual_static_d_transport network.id, 17, port_number, mac_lease, ip_lease, OpenFlowController::OFPP_LOCAL, ip_map[:network][:dhcp_server], 68
        end

        network.add_port port_number
        openflow_controller.update_network network.id

        openflow_controller.ovs_ofctl.add_flows_from_list openflow_controller.ports[port_number].queued_flows
        openflow_controller.ports[port_number].queued_flows.clear
      end
      ...
      def delete_instance_now port
        ...
        openflow_controller.networks.each { |network| network.remove_port port.port_info.number }
      end
        ...
        build_rule(rules).each do |rule|
          ...
          when 'tcp', 'udp'
            if rule[:ip_fport] == rule[:ip_tport]
              openflow_controller.install_static_transport 6, port.port_info.number, port.mac, port.ip, rule[:ip_fport], rule[:ip_source]
            elsif rule[:ip_fport] <= 1 and rule[:ip_tport] >= 65535
              openflow_controller.install_static_transport 6, port.port_info.number, port.mac, port.ip, 0, rule[:ip_source]
            else
          ...
          when 'icmp'
            ...
            #      iptables -p icmp -h
            openflow_controller.install_static_icmp rule[:icmp_type], rule[:icmp_code], port.port_info.number, port.mac, port.ip, rule[:ip_source]
          end
        end
      end
