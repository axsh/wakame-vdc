...
module Dcmgr::NodeModules
  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    ...
    initialize_hook do
      ...
      unix_socket = "#{Dcmgr.conf.ovs_run_dir}/br0.controller"
    ...
    def add_instance_now switch, port
      ...
      if !network.virtual
        ...
        port.init_instance_subnet(switch.eth_port,  mac_lease, ip_lease) if Dcmgr.conf.enable_subnet
      end
    ...
    def create_network network_map
      ...
      dcn = Dcmgr.conf.dc_networks[network_map[:physical_network][:name]]
      raise "Not found DC network in hva.conf: #{network_map[:physical_network][:name]}" if dcn.nil?
      raise "Bridge type is not capable of this network #{dcn.name}: #{dcn.bridge_type}" if dcn.bridge_type != 'ovs'
