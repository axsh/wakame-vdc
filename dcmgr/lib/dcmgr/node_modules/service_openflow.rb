...
module Dcmgr::NodeModules
  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    ...
    initialize_hook do
      ...
      if node.manifest.config.trema_tmp
        ENV['TREMA_TMP'] = node.manifest.config.trema_tmp
      else
    ...
    def add_eth_now switch, port
      ...
      raise "A single (and only a single) physical network must be registered: #{networks_map.inspect}." unless networks_map.one?
      ...
      if network.services.has_key?(:metadata_server) and network.services[:metadata_server].ip.to_s != Isono::Util.default_gw_ipaddr.to_s
        network.services[:metadata_server].request_mac(switch, network, port)
      end
    end
    ...
    def add_instance_now switch, port
      ...
      port_number = port.port_info.number
      network = get_network_from_map(vnic_map[:network])
      port.network = network
      ...
      if !network.virtual
        ...
        add_security_group(port, vnic_map[:instance_uuid])
        ...
        port.init_instance_subnet(switch.eth_port,  mac_lease, ip_lease) if node.manifest.config.enable_subnet
      end
    ...
    def add_tunnel_now switch, port
      ...
      raise "Tried to add a tunnel with GRE tunnels disabled." if !node.manifest.config.enable_gre
      return if not port.is_active
    ...
    def prefetch_physical_networks
      ...
      @networks_map = networks_map.select { |network| !network[:ipv4_gw].nil? and !network[:physical_network][:interface].nil? }
    end
    ...
    def create_network network_map
      ...
      network = Dcmgr::VNet::OpenFlow::OpenFlowNetwork.new(switch[1].datapath,
                                                           network_id,
                                                           network_map[:physical_network][:interface].nil?)
      ...
      if network_map[:ipv4_gw] and !network_map[:ipv4_gw].empty?
        network.services[:gateway] =
          Dcmgr::VNet::OpenFlow::ServiceGateway.new({:switch => switch[1],
                                                      :mac => openflow_controller.local_hw,
                                                      :ip => IPAddr.new(network_map[:ipv4_gw])})
      end
      
      if network.services[:dhcp].nil?
        network.services[:dhcp] =
          Dcmgr::VNet::OpenFlow::ServiceDhcp.new({:switch => switch[1], :mac => openflow_controller.local_hw})
      end
      
      if network.services[:dns].nil?
        network.services[:dns] =
          Dcmgr::VNet::OpenFlow::ServiceDns.new({:switch => switch[1], :mac => openflow_controller.local_hw})
      end
      
      if network.services[:dhcp].ip
        # Do nothing.
      elsif network_map[:dhcp_server] and !network_map[:dhcp_server].empty?
        network.services[:dhcp].ip = IPAddr.new(network_map[:dhcp_server])
      else
        network.services[:dhcp].ip = IPAddr.new(network_map[:ipv4_network]) | IPAddr.new("0.0.0.1") 
      end

      if network.services[:dns].ip
        # Do nothing.
      elsif network_map[:dns_server] and !network_map[:dns_server].empty?
        network.services[:dns].ip = IPAddr.new(network_map[:dns_server])
      end
      ...
      network.services.each { |key,service| service.install(network) }

      if !network.virtual
        logger.info "Creating physical network: id:#{network_id} link_interface:#{network_map[:link_interface]} physical:#{network_map[:physical_network][:interface]}."
        ...
        if network_map[:metadata_server] && network_map[:metadata_server_port] &&
            network_map[:metadata_server].to_s == Isono::Util.default_gw_ipaddr.to_s
          network.services[:metadata_server] =
            Dcmgr::VNet::OpenFlow::ServiceMetadata.new({ :switch => switch[1],
                                                         :of_port => Dcmgr::VNet::OpenFlow::OpenFlowController::OFPP_LOCAL,
                                                         :mac => @openflow_controller.local_hw,
                                                         :ip => IPAddr.new(network_map[:metadata_server]),
                                                         :listen_port => network_map[:metadata_server_port]})
          network.services[:metadata_server].install(network)
        end
        ...
        network.packet_handlers <<
          Dcmgr::VNet::OpenFlow::PacketHandler.new(Proc.new { |switch,port,message|
                                                     message.arp? and
                                                     message.arp_oper == Racket::L3::ARP::ARPOP_REQUEST and
                                                     message.arp_tpa.to_i == port.network.services[:dhcp].ip.to_i
                                                   }, Proc.new { |switch,port,message|
                                                     switch.datapath.send_arp(message.in_port, Racket::L3::ARP::ARPOP_REPLY,
        ...
        if node.manifest.config.enable_subnet
        ...
        event.subscribe("ovs/network_added/#{network_id}", '#') do |header, body|
          worker_thread.pass {
            ...
            if body[:host_ip] == Isono::Util.default_gw_ipaddr.to_s
              ...
              if node.manifest.config.enable_gre
              ...
              if body[:send_hosts] == true
                event.publish("ovs/network_added/#{network_id}", :args=>{
                                ...
                                :host_ip => Isono::Util.default_gw_ipaddr.to_s,
    ...
    def add_security_group port, inst_id
    ...
    def get_ports_from_instance_id inst_id
      ...
      # logger.debug "get_ports_from_instance_id: network_vifs:#{inst_map[:network_vifs].inspect}."
