...
module Isono::NodeModules
  class EventChannel
    def reset(evname, receiver_id=node.node_id)
...
module Dcmgr::NodeModules
  ...
  class ServiceOpenFlow < Isono::NodeModules::Base
    ...
    def create_network network_map
      ...
      logger.debug "subscribing to: 'vnet/network_service/#{network_id}'"

      event.subscribe("vnet/network_service/#{network_id}", '#') { |header, body|
        worker_thread.pass {
          logger.debug "vnet/network_service/#{network_id}: body:#{body.inspect}"

          case body[:status].to_sym
          when :add    then network.add_service(switch[1], body[:service])
          when :delete then network.delete_service(switch[1], body[:service])
          end
        }
      }
    ...
    def destroy_network network, remove_flows
      if !network.virtual
        ...
        event.reset("ovs/network_added/#{network.id}", '#')
      end

      event.reset("vnet/network_service/#{network.id}", '#')
    end
