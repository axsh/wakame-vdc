      ...
      def get_netfilter_data(node_id)
        ...
        secg_maps = Models::SecurityGroup.map { |secg|
          if secg_uuids.member? secg.canonical_uuid
            ...
            #Determine referenced security groups and the vnics in them
            referenced_groups = secg_map[:rules].map { |rule| rule[:ip_source].scan(/sg-\w+/).first }.compact.uniq
            referenced_groups.map! { |group_id|
              #p group_id
              #p get_instances_of_security_group(secg_map[:uuid])
              {
              :uuid => group_id,
              :vnics => get_instances_of_security_group(group_id).map { |inst_map| inst_map[:vif] }.flatten.uniq
              }
            }
            
            secg_map.merge({:foreign_vnics => foreign_vnics, :referenced_groups => referenced_groups})
          end
        }.compact
