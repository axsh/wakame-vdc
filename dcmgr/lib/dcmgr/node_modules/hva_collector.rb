  ...
  module NodeModules
    class HvaCollector < Isono::NodeModules::Base
      ...
      def get_netfilter_data(node_id)
        ...
        # Determine which security groups are in use and save vnics with none.
        secg_uuids = inst_maps.map { |inst_map|
          inst_map[:vif].map { |vif_map|
            empty_vnics[vif_map[:uuid]] = vif_map if vif_map[:security_groups].empty?
            vif_map[:security_groups]
          }
        }.flatten.uniq

        # Get the information of all security groups in use
        trimmed_uuids = secg_uuids.map {|id| Models::SecurityGroup.trim_uuid(id) }
        secg_maps = Models::SecurityGroup.filter(:uuid => trimmed_uuids).map { |secg|
