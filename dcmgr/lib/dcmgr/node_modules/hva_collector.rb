  ...
  module NodeModules
    class HvaCollector < Isono::NodeModules::Base
      ...
      def get_nic(nic_uuid)
        ...
        hash = nic.to_hash
        hash[:network] = nic.network.to_hash
        hash[:network_port] = nic.network_port.first.nil? ? nil : nic.network_port.first.to_hash
        hash
      end
      ...
      def prepare_host_node_vnet(host_node_uuid, network_uuid)
        ...
        logger.debug "host_node_vnet: vnet:#{vnet.inspect} vnets:#{host_node.vnet.inspect}."

        if vnet.nil? or vnet.empty?
          network = Models::Network[network_uuid]
          raise "UnknownNetworkID" if network.nil?

          host_node_vnet = host_node.add_vnet(network)
        else
          host_node_vnet = vnet.first
        end

        host_node_vnet.to_api_document
      end
