  ...
  module NodeModules
    class HvaCollector < Isono::NodeModules::Base
      ...
      def get_instance_monitor_data(node_id)
        ...
        Models::Instance.filter(:host_node_id => hp && hp.id, :state => "running").all.map { |inst|
          {
      ...
      def get_netfilter_data(node_id)
        ...
        local_vnic_maps  = local_vnics_dataset.all.map {|vnic| vnic.to_hash } 
          ...
          if vnic_map[:security_groups].empty?
            ...
            vnic_map[:security_groups].each { |group_id|
              if security_groups[group_id] == nil
                ...
                security_groups[group_id] = secg.to_hash

                # Prepare the hash that stores this group's vnics on the current host
                security_groups[group_id][:local_vnics] = {}
                # Save all the vnics on other host nodes
                security_groups[group_id][:foreign_vnics] = secg.network_vif_dataset.exclude(:uuid => local_vnics_dataset.select(:uuid)).all.map {|sg| sg.to_hash}
                #Models::NetworkVif.filter(:security_groups => secg).exclude(local_vnics_dataset).all.map {|sg| sg.to_hash}
                # Save all vnics in security groups that are referenced by this group
                security_groups[group_id][:referencees] = {}
                secg.referencees.each { |ref|
                  security_groups[group_id][:referencees][ref.canonical_uuid] = secg.network_vif_dataset.filter(:instance => Models::Instance.runnings.exclude(:host_node => Models::HostNode.filter(:node_id => node_id))).all.map {|sg| sg.to_hash }
                }
                # Save all vnics in security groups that are referencing this group
                security_groups[group_id][:referencers] = {}
                secg.referencers.each { |ref|
                  security_groups[group_id][:referencers][ref.canonical_uuid] = secg.network_vif_dataset.filter(:instance => Models::Instance.runnings.exclude(:host_node => Models::HostNode.filter(:node_id => node_id))).all.map {|sg| sg.to_hash }
                }
              end

              security_groups[group_id][:local_vnics][vnic_map[:uuid]] = vnic_map
            }
          end
        }
