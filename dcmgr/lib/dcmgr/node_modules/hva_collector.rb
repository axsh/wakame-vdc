    ...
    class HvaCollector < Isono::NodeModules::Base
      ...
      def get_group_instance_ipv4s(instance_id,set = :inside)
        ...
        inst.netfilter_groups.compact.map { |netfilter_group|
          ...
          netfilter_group.instances_dataset.runnings.all.compact.map { |instance|
      ...
      def get_instances_of_netfilter_group(netfilter_group_uuid)
        ng_map = Models::NetfilterGroup[netfilter_group_uuid]
        raise "UnknownNetfilterGroupID" if ng_map.nil?
        # do not include 'stopped' or 'scheduling' instances as they
        # are not allocated IP address.
        inst_maps = ng_map.instances_dataset.runnings.all.map { |inst| inst.to_hash }.flatten.uniq.compact
        inst_maps
      end
