  ...
  module NodeModules
    class HvaCollector < Isono::NodeModules::Base
      ...
      def update_instance(instance_id, data)
        Models::Instance.lock!
        inst = Models::Instance[instance_id]
        inst.set_fields(data, [:state, :terminated_at]).save
      ...
      def get_instances_of_netfilter_group(netfilter_group_id)
        ...
        g.instance_netfilter_groups.map { |instance_netfilter_group|
          instance_netfilter_group.instance_dataset.lives.all.map { |inst| p inst.to_hash[:state] }
