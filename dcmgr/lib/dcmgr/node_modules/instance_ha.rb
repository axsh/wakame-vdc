  ...
  module NodeModules
    class InstanceHA < Isono::NodeModules::Base
      ...
      def restart_instance(inst)
        ...
        begin
          jobrpc.run("hva-handle.#{inst.host_pool.node_id}", 'cleanup', inst.canonical_uuid)
        rescue => e
        ...
        when Models::Image::BOOT_DEV_SAN
          ...
          res = jobrpc.submit("hva-handle.#{inst.host_pool.node_id}", 'run_vol_store', inst.canonical_uuid, boot_vol.canonical_uuid)
        when Models::Image::BOOT_DEV_LOCAL
          res = jobrpc.submit("hva-handle.#{inst.host_pool.node_id}", 'run_local_store', inst.canonical_uuid)
        else
