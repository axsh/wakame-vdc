    ...
    class InstanceMonitor < Isono::NodeModules::Base
      ...
      def instance_state_recovery(instance, running)
        if running
          ...
          when Const::Instance::STATE_HALTING, Const::Instance::STATE_SHUTTING_DOWN
            # TODO: need to add columns to instances table.
            #if (Time.now - instance[:last_state_updated_at]) > instance[:guest_poweroff_timeout_sec].to_f
            # guess time gap to give up waiting from state file time stamp.
            s = File.stat(File.expand_path("#{instance[:uuid]}/state", Dcmgr.conf.vm_data_dir))
            if (Time.now - s.mtime) > Dcmgr.conf.default_soft_poweroff_timeout_sec.to_f
              logger.info("Giving up waiting for halting instance and push back state for #{instance[:uuid]}:  #{instance[:state]} -> running")
              rpc.request('hva-collector', 'update_instance', instance[:uuid], {:state=>Const::Instance::STATE_RUNNING}) { |req|
                req.oneshot = true
              }
            end
          when Const::Instance::STATE_HALTED, Const::Instance::STATE_STARTING
            logger.info("Force push back state for #{instance[:uuid]}:  #{instance[:state]} -> running")
            rpc.request('hva-collector', 'update_instance', instance[:uuid], {:state=>Const::Instance::STATE_RUNNING}) { |req|
              req.oneshot = true
            }
          end
