...
module Dcmgr::NodeModules
  class Maintenance < Isono::NodeModules::Base
    ...
    def automatic_windows_password_deletion
      ...
      Dcmgr::Models::Instance.dataset.alives.where('password_will_be_deleted_at IS NOT NULL').password_deletion_time_passed.each { |handle|
        logger.debug "Automatically deleting encrypted password for instance with uuid:#{handle.canonical_uuid}"
        handle.delete_windows_password
      }
    end
