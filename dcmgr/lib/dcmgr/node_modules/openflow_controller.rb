    ...
    class OpenFlowController < Trema::Controller
      ...
      def packet_in datapath_id, message
        ...
        if message.ipv4? and message.tcp?
          logger.info "Got IPv4/TCP packet; #{message.ipv4_daddr.to_s}:#{message.tcp_dst_port} #{message.ipv4_saddr.to_s}:#{message.tcp_src_port}."
        end

        # Rewrite this after adding ip/tcp get functions to PacketIn.
        if message.ipv4? and message.tcp? and message.ipv4_daddr.to_s == "169.254.169.254" and message.tcp_dst_port == 80
          install_dnat_entry datapath_id, message, 0x2108, OFPP_LOCAL, @local_hw, Isono::Util.default_gw_ipaddr, 9002
      ...
      def install_dnat_entry datapath_id, message, priority, dest_port, dest_hw, dest_ip, dest_tp
        ...
        prefix_outgoing = "#{prefix},#{@ovs_ofctl.arg_in_port message.in_port}"
        match_outgoing = "dl_src=#{message.macsa.to_s},dl_dst=#{message.macda.to_s},nw_src=#{msg_nw_src},nw_dst=#{msg_nw_dst},tp_src=#{message.tcp_src_port},tp_dst=#{message.tcp_dst_port}"
        action_outgoing = "mod_dl_dst:#{dest_hw},mod_nw_dst:#{dest_ip},mod_tp_dst:#{dest_tp},#{@ovs_ofctl.arg_output dest_port}"

        prefix_incoming = "#{prefix},#{@ovs_ofctl.arg_in_port dest_port}"
        match_incoming = "dl_src=#{dest_hw.to_s},dl_dst=#{message.macsa.to_s},nw_src=#{dest_ip},nw_dst=#{msg_nw_src},tp_src=#{dest_tp},tp_dst=#{message.tcp_src_port}"
