    ...
    class OpenFlowSwitch
      ...
      def features_reply message
        ...
        message.ports.each do | each |
          ...
          datapath.controller.insert_port message.datapath_id, port
        end
...
#
    ...
    class OpenFlowController < Trema::Controller
      ...
      def switch_ready datapath_id
        raise "Already have switch." if switches.has_key? datapath_id

        logger.info "switch_ready from %#x." % datapath_id

        ofctl = @ovs_ofctl.dup
        ofctl.switch_name = 'br0'

        switches[datapath_id] = OpenFlowSwitch.new(OpenFlowDatapath.new(self, datapath_id, ofctl), 'br0')
      ...
      def insert_port datapath_id, port
      ...
      def packet_in datapath_id, message
        ...
        if message.ipv4? and message.tcp?
          ...
          if message.ipv4_daddr.to_s == "169.254.169.254" and message.tcp_dst_port == 80
            install_dnat_entry datapath_id, message, TABLE_METADATA_OUTGOING, TABLE_METADATA_INCOMING, OFPP_LOCAL, local_hw, Isono::Util.default_gw_ipaddr, 9002
      ...
      def install_route port, hw, ip
        ...
        port.queue_flow "priority=#{1}", "table=#{TABLE_MAC_ROUTE},dl_dst=#{hw}", "output:#{port.port_info.number}"

        port.queue_flow "priority=#{2}", "table=#{0},in_port=#{port.port_info.number},dl_src=#{hw}", "resubmit(,#{TABLE_ROUTE_DIRECTLY})"
        port.queue_flow "priority=#{1}", "table=#{TABLE_ROUTE_DIRECTLY},dl_dst=#{hw}", "output:#{port.port_info.number}"

        # @ovs_ofctl.add_flow "priority=#{1},table=#{TABLE_LOAD_DST},dl_dst=#{hw}", "load:#{port.port_info.number}->NXM_NX_REG0[],resubmit(,#{TABLE_LOAD_SRC})"
        port.queue_flow "priority=#{1}", "table=#{TABLE_LOAD_DST},dl_dst=#{hw}", "drop"
      end

      def install_virtual_route network, port, hw, ip
        port.queue_flow "priority=#{7}", "table=#{0},in_port=#{port.port_info.number}", "load:#{network.id}->NXM_NX_REG1[],resubmit(,#{TABLE_VIRTUAL_SRC})"
        port.queue_flow "priority=#{2}", "table=#{TABLE_VIRTUAL_DST},reg1=#{network.id},dl_dst=#{hw}", "output:#{port.port_info.number}"
      end
      ...
      def install_arp_antispoof port, hw, ip
        ...
        port.queue_flow "priority=#{3}", "table=#{TABLE_ARP_ANTISPOOF},arp,in_port=#{port.port_info.number},arp_sha=#{hw},nw_src=#{ip}", "resubmit(,#{TABLE_ARP_ROUTE})"
        port.queue_flow "priority=#{2}", "table=#{TABLE_ARP_ANTISPOOF},arp,arp_sha=#{hw}", "drop"
        port.queue_flow "priority=#{2}", "table=#{TABLE_ARP_ANTISPOOF},arp,nw_src=#{ip}", "drop"

        # Routing of ARP packets to instance.
        port.queue_flow "priority=#{2}", "table=#{TABLE_ARP_ROUTE},arp,dl_dst=#{hw},nw_dst=#{ip}", "output:#{port.port_info.number}"
      end

      def install_virtual_arp_route network, in_port, hw, ip, out_port, remote_hw, remote_ip
        # Require correct ARP source IP/MAC from instance, and protect the instance IP from ARP spoofing.
        # port.queue_flow "priority=#{4}", "table=#{TABLE_VIRTUAL_DST},arp,reg1=#{network.id},arp_sha=#{hw},nw_src=#{ip},dl_dst=#{remote_hw},nw_dst=#{remote_ip}", "output:#{out_port}"
        # port.queue_flow "priority=#{3}", "table=#{TABLE_VIRTUAL_DST},arp,arp_sha=#{hw}", "drop"
        # port.queue_flow "priority=#{3}", "table=#{TABLE_VIRTUAL_DST},arp,nw_src=#{ip}", "drop"

        # Make generic...
        add_flow in_port, "priority=#{4}", "table=#{TABLE_VIRTUAL_DST},arp,reg1=#{network.id},dl_dst=#{remote_hw},nw_dst=#{remote_ip}", "output:#{out_port}"

        # Routing of ARP packets to instance.
        add_flow in_port, "priority=#{2}", "table=#{TABLE_ARP_ROUTE},in_port=#{out_port},arp,dl_dst=#{hw},nw_dst=#{ip}", "output:#{in_port}"
      end
      ...
      def install_static_icmp icmp_type, icmp_code, port, local_hw, local_ip, src_ip
        ...
        incoming_actions = "load:#{port.port_info.number}->NXM_NX_REG0[],resubmit(,#{TABLE_LOAD_SRC})"
        port.queue_flow "priority=#{3}", incoming_match, incoming_actions

        outgoing_match = "table=#{TABLE_LOAD_SRC},#{match_type},in_port=#{port.port_info.number},dl_src=#{local_hw},nw_src=#{local_ip}#{src_ip_match},"
      ...
      def install_static_transport nw_proto, port, local_hw, local_ip, local_port, remote_ip
        ...
        outgoing_match = "table=#{TABLE_LOAD_SRC},#{match_type},in_port=#{port.port_info.number},dl_src=#{local_hw},nw_src=#{local_ip}#{dst_match}"
        outgoing_actions = "output:NXM_NX_REG0[]"
        port.queue_flow "priority=#{3}", outgoing_match, outgoing_actions
      end
      ...
      def install_virtual_static_d_transport network, nw_proto, in_port, local_hw, local_ip, out_port, remote_ip, remote_port
        ...
        outgoing_match = "table=#{TABLE_VIRTUAL_SRC},reg1=#{network.id},#{match_type},dl_src=#{local_hw},nw_src=#{local_ip}#{dst_match}"
      ...
      def install_local_icmp port, hw, ip
        ...
        port.queue_flow "priority=#{1}", "table=#{TABLE_LOAD_SRC},#{match_type},in_port=#{port.port_info.number},dl_src=#{hw},nw_src=#{ip}", actions
      end
      ...
      def install_virtual_network network
        ...
        @ovs_ofctl.add_flow "priority=#{2},table=#{TABLE_VIRTUAL_SRC},reg1=#{network.id},reg2=#{0}", "resubmit\\(,#{TABLE_VIRTUAL_DST}\\)"
        @ovs_ofctl.add_flow "priority=#{1},table=#{TABLE_VIRTUAL_SRC},reg1=#{network.id},arp", "learn\\(#{learn_arp_match},#{learn_arp_actions}\\),resubmit\\(,#{TABLE_VIRTUAL_DST}\\)"
        @ovs_ofctl.add_flow "priority=#{0},table=#{TABLE_VIRTUAL_SRC},reg1=#{network.id}", "resubmit\\(,#{TABLE_VIRTUAL_DST}\\)"

        # Catch ARP for the DHCP server.
        @ovs_ofctl.add_flow "priority=#{3},table=#{TABLE_VIRTUAL_DST},reg1=#{network.id},arp,nw_dst=#{network.dhcp_ip.to_s}", "controller"

        # Catch DHCP requests.
        @ovs_ofctl.add_flow "priority=#{3},table=#{TABLE_VIRTUAL_DST},reg1=#{network.id},udp,dl_dst=#{network.dhcp_hw},nw_dst=#{network.dhcp_ip.to_s},tp_src=68,tp_dst=67", "controller"
        @ovs_ofctl.add_flow "priority=#{3},table=#{TABLE_VIRTUAL_DST},reg1=#{network.id},udp,dl_dst=ff:ff:ff:ff:ff:ff,nw_dst=255.255.255.255,tp_src=68,tp_dst=67", "controller"
      ...
      def install_gre_tunnel network_id, port
        port.port_type = OpenFlowPort::PORT_TYPE_TUNNEL
        port.queue_flow "priority=#{7}", "table=#{0},in_port=#{port.port_info.number}", "load:#{network_id}->NXM_NX_REG1[],load:#{port.port_info.number}->NXM_NX_REG2[],resubmit(,#{TABLE_VIRTUAL_SRC})"
      end

      def install_eth port
        port.port_type = OpenFlowPort::PORT_TYPE_ETH
        ...
        port.queue_flow "priority=#{2}", "in_port=#{port_number}",  "resubmit(,#{TABLE_ROUTE_DIRECTLY})"
        port.queue_flow "priority=#{0}", "table=#{TABLE_MAC_ROUTE}", "output:#{port_number}"
        port.queue_flow "priority=#{0}", "table=#{TABLE_ROUTE_DIRECTLY}", "output:#{port_number}"
        port.queue_flow "priority=#{0}", "table=#{TABLE_LOAD_DST}", "load:#{port_number}->NXM_NX_REG0[],resubmit(,#{TABLE_LOAD_SRC})"
        port.queue_flow "priority=#{4}", "table=#{TABLE_LOAD_SRC},in_port=#{port_number}", "output:NXM_NX_REG0[]"
        port.queue_flow "priority=#{1}", "table=#{TABLE_ARP_ANTISPOOF},arp,in_port=#{port_number}", "resubmit(,#{TABLE_ARP_ROUTE})"
        port.queue_flow "priority=#{0}", "table=#{TABLE_ARP_ROUTE},arp", "output:#{port_number}"
        port.queue_flow "priority=#{4}", "table=#{TABLE_METADATA_OUTGOING},in_port=#{port_number}", "drop"
      end
