  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def features_reply message
        ...
        @ovs_ofctl.add_flow "priority=#{0x3001},table=#{0x0},arp,in_port=#{1}", "resubmit\\(,#{0x10}\\)"
        @ovs_ofctl.add_flow "priority=#{0x3001},table=#{0x0},arp,local", "resubmit\\(,#{0x10}\\)"
        @ovs_ofctl.add_flow "priority=#{0x3000},table=#{0x0},arp", "drop"

        # Flow table for routing packets after without any filtering
        # or translation.
        @ovs_ofctl.add_flow "priority=#{0x1000},table=#{0x10},dl_dst=FF:FF:FF:FF:FF:FF", "flood"
        @ovs_ofctl.add_flow "priority=#{0x1000},table=#{0x10},dl_dst=#{@local_hw.to_s}", "local"
        @ovs_ofctl.add_flow "priority=#{0x0000},table=#{0x10}", "output:1"
      ...
      def add_instance datapath_id, port_info
        ...
        if not @service_openflow.host_viftable_map.has_key? port_info.name
          EM.add_timer(10) { add_instance datapath_id, port_info }
        ...
        send_flow_mod_add(datapath_id,
                          :match => Trema::Match.new( :dl_type => 0x0806, :dl_dst => inside_hw ),
                          :priority => 0x2002,
                          :actions => Trema::ActionOutput.new( :port => port_info.number ) )

        send_flow_mod_add(datapath_id,
                          :match => Trema::Match.new( :dl_dst => inside_hw ),
                          :priority => 0x1002,
                          :actions => Trema::ActionOutput.new( :port => port_info.number ) )

        @ovs_ofctl.add_flow "priority=#{0x3001},table=#{0x0},arp,in_port=#{port_info.number},arp_sha=#{inside_hw},nw_src=#{inside_ip}", "resubmit\\(,#{0x10}\\)"
        # Also filter incoming arp packets?

        @ovs_ofctl.add_flow "priority=#{0x1000},table=#{0x10},dl_dst=#{inside_hw}", "output:#{port_info.number}"
      ...
      def install_arp_antispoof datapath_id, priority, in_port, dl_src, nw_src
        # send_flow_mod_add(datapath_id,
        #                   :match => Trema::Match.new( :in_port => in_port, :dl_type => 0x0800, :nw_proto => 6,
        #                                               :dl_src => dl_src.to_s, :nw_src => nw_src ),
        #                   :priority => priority,
        #                   :actions => [ ] )
