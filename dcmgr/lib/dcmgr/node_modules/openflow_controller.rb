  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def features_reply message
        ...
        message.ports.each do | each |
          ...
          if each.number >= OFPP_MAX
            ...
            @service_openflow.add_tunnel ports[each.number]
          else
          end
        end
      ...
      def packet_in datapath_id, message
        ...
        if message.arp?
          ...
          if message.arp_oper == Racket::L2::Ethernet::ARPOP_REQUEST and message.arp_tpa.to_i == port.network.dhcp_ip.to_i
            send_arp(datapath_id, message.in_port, Racket::L2::Ethernet::ARPOP_REPLY, port.network.dhcp_hw.to_s, port.network.dhcp_ip.to_s, message.macsa.to_s, message.arp_spa.to_s)
          end
      ...
      def install_virtual_network network_id, dhcp_hw, dhcp_ip
        ...
        learn_arp_match = "priority=#{1},idle_timeout=#{3600*10},table=#{@@virtual_dest},reg1=#{network_id},NXM_OF_ETH_DST[]=NXM_OF_ETH_SRC[]"
        learn_arp_actions = "output:NXM_NX_REG2[]"

        @ovs_ofctl.add_flow "priority=#{2},table=#{@@virtual_src},reg1=#{network_id},reg2=#{0}", "resubmit\\(,#{@@virtual_dest}\\)"
        @ovs_ofctl.add_flow "priority=#{1},table=#{@@virtual_src},reg1=#{network_id},arp", "learn\\(#{learn_arp_match},#{learn_arp_actions}\\),resubmit\\(,#{@@virtual_dest}\\)"
        @ovs_ofctl.add_flow "priority=#{0},table=#{@@virtual_src},reg1=#{network_id}", "resubmit\\(,#{@@virtual_dest}\\)"

        # Catch ARP for the DHCP server.
        @ovs_ofctl.add_flow "priority=#{2},table=#{@@virtual_dest},reg1=#{network_id},arp,nw_dst=#{dhcp_ip.to_s}", "controller"

        # Catch DHCP requests.
        @ovs_ofctl.add_flow "priority=#{2},table=#{@@virtual_dest},reg1=#{network_id},udp,dl_dst=#{dhcp_hw},nw_dst=#{dhcp_ip.to_s},tp_src=68,tp_dst=67", "controller"
        ...
        logger.info "installed virtual network: network_id:#{network_id} dhcp_hw:#{dhcp_hw} dhcp_ip:#{dhcp_ip.to_s}."
      end
      ...
      def install_gre_tunnel network_id, port
        ...
        # port.queue_flow "priority=#{7}", "table=#{0},in_port=#{port.port_info.number}", "load:NXM_NX_TUN_ID_W[]->NXM_NX_REG1[],load:#{port.port_info.number}->NXM_NX_REG2[],resubmit(,#{@@virtual_src})"
        # port.queue_flow "priority=#{7}", "table=#{0},in_port=#{port.port_info.number},tun_id=#{network_id}", "load:#{network_id}->NXM_NX_REG1[],load:#{port.port_info.number}->NXM_NX_REG2[],resubmit(,#{@@virtual_src})"
    ...
    class OvsOfctl
      ...
      def add_gre_tunnel tunnel_name, remote_ip
        system("#{@ovs_vsctl} add-port #{@switch} #{tunnel_name} -- set interface #{tunnel_name} type=gre options:remote_ip=#{remote_ip}")
      end
