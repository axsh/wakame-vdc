  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def add_instance datapath_id, port_info
        ...
        @ovs_ofctl.add_flow "priority=#{1},table=#{11},    dl_dst=#{inside_hw}", "output:#{port_info.number}"

        # Require correct ARP source IP/MAC from instance, and protect the instance IP from ARP spoofing.
        @ovs_ofctl.add_flow "priority=#{3},table=#{ 2},arp,in_port=#{port_info.number},arp_sha=#{inside_hw},nw_src=#{inside_ip}", "resubmit\\(,#{12}\\)"
        @ovs_ofctl.add_flow "priority=#{2},table=#{ 2},arp,nw_src=#{inside_ip}", "drop"

        # Routing of ARP packets to instance.
        @ovs_ofctl.add_flow "priority=#{2},table=#{12},arp,dl_dst=#{inside_hw},nw_dst=#{inside_ip}", "output:#{port_info.number}"
