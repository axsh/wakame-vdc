  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def features_reply message
        ...
        # network.
        flows << ["priority=#{6},udp,in_port=#{@eth_number},dl_dst=ff:ff:ff:ff:ff:ff,nw_src=0.0.0.0,nw_dst=255.255.255.255,tp_src=68,tp_dst=67", "local"]
      ...
      def install_virtual_route network_id, port, hw, ip
        ...
        add_flow port, "priority=#{4}", "table=#{0},in_port=#{port}", "load:#{network.id}->NXM_NX_REG1[],resubmit(,#{@@virtual_src})"
        ...
        # Replace old broadcast flow.
        network.add_port port
        add_flow port, "priority=#{1}", "table=#{@@virtual_dest},reg1=#{network.id},dl_dst=ff:ff:ff:ff:ff:ff", network.generate_flood_actions
      end
