  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def features_reply message
        ...
        networks[1].add_port OFPP_LOCAL, true
        networks[1].add_port @eth_number, true
      ...
      def install_virtual_route network_id, port, hw, ip
        ...
        port.queue_flow "priority=#{2}", "table=#{@@virtual_dest},reg1=#{network_id},dl_dst=#{hw}", "output:#{port.port_info.number}"
      end
      ...
      def install_virtual_network network_id, dhcp_hw, dhcp_ip
        ...
        networks[network_id].flood_flows       << ["priority=#{1},table=#{@@virtual_dest},reg1=#{network_id},reg2=#{0},dl_dst=ff:ff:ff:ff:ff:ff", "", "output:<>", ""]
        networks[network_id].flood_local_flows << ["priority=#{0},table=#{@@virtual_dest},reg1=#{network_id},dl_dst=ff:ff:ff:ff:ff:ff", "", "output:<>", ""]

        learn_arp_match = "priority=#{1},idle_timeout=#{3600*10},table=#{@@virtual_dest},reg1=#{network_id},reg2=#{0},NXM_OF_ETH_DST[]=NXM_OF_ETH_SRC[]"
        ...
        @ovs_ofctl.add_flow "priority=#{3},table=#{@@virtual_dest},reg1=#{network_id},arp,nw_dst=#{dhcp_ip.to_s}", "controller"

        # Catch DHCP requests.
        @ovs_ofctl.add_flow "priority=#{3},table=#{@@virtual_dest},reg1=#{network_id},udp,dl_dst=#{dhcp_hw},nw_dst=#{dhcp_ip.to_s},tp_src=68,tp_dst=67", "controller"
        @ovs_ofctl.add_flow "priority=#{3},table=#{@@virtual_dest},reg1=#{network_id},udp,dl_dst=ff:ff:ff:ff:ff:ff,nw_dst=255.255.255.255,tp_src=68,tp_dst=67", "controller"

        logger.info "installed virtual network: network_id:#{network_id} dhcp_hw:#{dhcp_hw} dhcp_ip:#{dhcp_ip.to_s}."
      end

      def install_physical_network network_id
        networks[network_id] = OpenFlowNetwork.new(network_id)
        networks[network_id].flood_flows << ["priority=#{1},table=#{@@mac_route_table},dl_dst=FF:FF:FF:FF:FF:FF", "", "output:<>", ""]
        networks[network_id].flood_flows << ["priority=#{1},table=#{@@route_directly},dl_dst=FF:FF:FF:FF:FF:FF", "", "output:<>", ""]
        networks[network_id].flood_flows << ["priority=#{1},table=#{@@load_dest_table},dl_dst=FF:FF:FF:FF:FF:FF", "", "load:<>->NXM_NX_REG0[],resubmit(,#{@@send_src_table})", ""]
        networks[network_id].flood_flows << ["priority=#{1},table=#{@@arp_route_table},arp,dl_dst=FF:FF:FF:FF:FF:FF,arp_tha=00:00:00:00:00:00", "", "output:<>", ""]
      end
