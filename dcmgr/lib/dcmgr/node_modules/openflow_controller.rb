...
#
    ...
    class OpenFlowPort
      ...
      def init_gre_tunnel
        @port_type = OpenFlowPort::PORT_TYPE_TUNNEL
      ...
      def install_instance_route hw, ip
        port_type = OpenFlowPort::PORT_TYPE_INSTANCE
        ...
        # @ovs_ofctl.add_flow "priority=#{1},table=#{TABLE_LOAD_DST},dl_dst=#{hw}", "load:#{port_info.number}->NXM_NX_REG0[],resubmit(,#{TABLE_LOAD_SRC})"
        queue_flow "priority=#{1}", "table=#{TABLE_LOAD_DST},dl_dst=#{hw}", "drop"
      end

      def install_virtual_route hw, ip
        port_type = OpenFlowPort::PORT_TYPE_INSTANCE
    ...
    class OpenFlowController < Trema::Controller
      ...
      def port_status message
        ...
        when OFPPR_ADD
          ...
          switch = switches[message.datapath_id]

          port = OpenFlowPort.new(switch.datapath, message.phy_port)
          port.is_active = true
          ports[message.phy_port.number] = port

          insert_port switch, port
