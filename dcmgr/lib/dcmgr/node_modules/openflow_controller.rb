    ...
    class OpenFlowController < Trema::Controller
      ...
      def packet_in datapath_id, message
        ...
        if message.arp?
          ...
          raw_out.l2.src_mac = port.network.dhcp_hw.to_s
          raw_out.l2.dst_mac = message.macsa.to_s
            
          raw_out.l3 = Racket::L3::ARP.new
          raw_out.l3.opcode = Racket::L2::Ethernet::ARPOP_REPLY
          raw_out.l3.sha = port.network.dhcp_hw.to_s
          raw_out.l3.spa = port.network.dhcp_ip
          raw_out.l3.tha = message.arp_sha.to_s
          raw_out.l3.tpa = message.arp_spa.to_s

          raw_out.layers.compact.each { |l|
            logger.debug "ARP packet: layer:#{l.pretty}."
          }

          send_packet_out(datapath_id, :data => raw_out.pack, :actions => Trema::ActionOutput.new( :port => message.in_port ) )
          return
        end
        ...
        if message.ipv4? and message.udp?
          ...
          if message.udp_src_port == 68 and message.udp_dst_port == 67 # and message.ipv4_daddr.to_s == "10.100.0.1"
            ...
            raw_out.l2 = Racket::L2::Ethernet.new
            raw_out.l2.src_mac = port.network.dhcp_hw.to_s
            raw_out.l2.dst_mac = port.mac.to_s
            
            raw_out.l3 = Racket::L3::IPv4.new
            raw_out.l3.src_ip = port.network.dhcp_ip
            raw_out.l3.dst_ip = port.ip
            raw_out.l3.protocol = 0x11

            raw_out.l4 = Racket::L4::UDP.new
            raw_out.l4.src_port = 67
            raw_out.l4.dst_port = 68
            raw_out.l4.payload = dhcp_out.pack

            raw_out.l4.fix!(raw_out.l3.src_ip, raw_out.l3.dst_ip)

            raw_out.layers.compact.each { |l|
              logger.debug "DHCP packet: layer:#{l.pretty}."
            }
