  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def switch_ready datapath_id
        ...
        networks[1] = OpenFlowNetwork.new(1)
        networks[1].flood_flows << ["priority=#{1},table=#{@@mac_route_table},dl_dst=FF:FF:FF:FF:FF:FF", "", "output:<>", ""]
        networks[1].flood_flows << ["priority=#{1},table=#{@@route_directly},dl_dst=FF:FF:FF:FF:FF:FF", "", "output:<>", ""]
        networks[1].flood_flows << ["priority=#{1},table=#{@@load_dest_table},dl_dst=FF:FF:FF:FF:FF:FF", "", "load:<>->NXM_NX_REG0[],resubmit(,#{@@send_src_table})", ""]
        networks[1].flood_flows << ["priority=#{1},table=#{@@arp_route_table},arp,dl_dst=FF:FF:FF:FF:FF:FF,arp_tha=00:00:00:00:00:00", "", "output:<>", ""]
      ...
      def features_reply message
        ...
        message.ports.each do | each |
          ...
          if each.number >= OFPP_MAX
            ...
            logger.error "Multiple 'eth' interfaces are part of the bridge." if not @eth_number.nil?
            @eth_number = each.number
          elsif each.name =~ /^vif-/
            @service_openflow.add_instance ports[each.number]
          else
          end
        end

        raise MissingPort, "Missing port: No 'eth' interface in bridge." if @eth_number.nil?

        networks[1].add_port OFPP_LOCAL
        networks[1].add_port @eth_number
        ...
        # network.
        flows << ["priority=#{6},udp,in_port=local,dl_dst=ff:ff:ff:ff:ff:ff,nw_src=0.0.0.0,nw_dst=255.255.255.255,tp_src=68,tp_dst=67", "output:#{@eth_number}"]
        ...
        flows << ["priority=#{2},in_port=#{@eth_number}",  "resubmit(,#{@@route_directly})"]
        ...
        # Unprotected flows.
        # flows << ["priority=#{1},table=#{@@mac_route_table},dl_dst=FF:FF:FF:FF:FF:FF", "flood"]
        flows << ["priority=#{1},table=#{@@mac_route_table},dl_dst=#{@local_hw.to_s}", "local"]
        flows << ["priority=#{0},table=#{@@mac_route_table}", "output:#{@eth_number}"]

        # Protected flows.
        # flows << ["priority=#{1},table=#{@@route_directly},dl_dst=FF:FF:FF:FF:FF:FF", "flood"]
        flows << ["priority=#{1},table=#{@@route_directly},dl_dst=#{@local_hw.to_s}", "local"]
        flows << ["priority=#{0},table=#{@@route_directly}", "output:#{@eth_number}"]

        # flows << ["priority=#{1},table=#{@@load_dest_table},dl_dst=FF:FF:FF:FF:FF:FF", "load:#{OFPP_FLOOD}->NXM_NX_REG0[],resubmit(,#{@@send_src_table})"]
        flows << ["priority=#{1},table=#{@@load_dest_table},dl_dst=#{@local_hw.to_s}", "load:#{OFPP_LOCAL}->NXM_NX_REG0[],resubmit(,#{@@send_src_table})"]
        flows << ["priority=#{0},table=#{@@load_dest_table}", "load:#{@eth_number}->NXM_NX_REG0[],resubmit(,#{@@send_src_table})"]
        ...
        flows << ["priority=#{4},table=#{@@send_src_table},in_port=#{@eth_number}", "output:NXM_NX_REG0[]"]
        ...
        flows << ["priority=#{1},table=#{@@arp_antispoof_table},arp,in_port=#{@eth_number}", "resubmit(,#{@@arp_route_table})"]
        ...
        # flows << ["priority=#{1},table=#{@@arp_route_table},arp,dl_dst=FF:FF:FF:FF:FF:FF,arp_tha=00:00:00:00:00:00", "flood"]
        flows << ["priority=#{1},table=#{@@arp_route_table},arp,dl_dst=#{@local_hw.to_s}", "local"]
        flows << ["priority=#{0},table=#{@@arp_route_table},arp", "output:#{@eth_number}"]
        ...
        flows << ["priority=#{4},table=#{@@metadata_outgoing_table},in_port=#{@eth_number}", "drop"]
        ...
        # flows << ["priority=#{4},table=#{@@metadata_incoming_table},tcp,in_port=#{@eth_number}", "drop"]
        # flows << ["priority=#{0},table=#{@@metadata_incoming_table},tcp", "drop"]

        @ovs_ofctl.add_flows_from_list flows        
        networks.each { |network| @ovs_ofctl.add_flows_from_list network[1].generate_flood_flows }
      end
      ...
      def port_status message
        ...
        when OFPPR_DELETE
          ...
          @ovs_ofctl.del_flows_from_list ports[message.phy_port.number].active_flows

          ports.delete message.phy_port.number

          raise "CouldNotDeleteOpenflowPort" if ports.has_key? message.phy_port.number
      ...
      def packet_in datapath_id, message
        if message.arp?
          logger.debug "Got ARP packet; source:#{message.arp_sha.to_s}:#{message.arp_spa.to_s} dest:#{message.arp_tha.to_s}:#{message.arp_tpa.to_s}."
          return
        end

        if message.ipv4? and message.tcp?
          logger.debug "Got IPv4/TCP packet; source:#{message.ipv4_saddr.to_s}:#{message.tcp_src_port} dest:#{message.ipv4_daddr.to_s}:#{message.tcp_dst_port}."
        ...
        if message.ipv4? and message.udp?
          logger.debug "Got IPv4/UDP packet; source:#{message.ipv4_saddr.to_s}:#{message.udp_src_port} dest:#{message.ipv4_daddr.to_s}:#{message.udp_dst_port}."

          if message.udp_src_port == 68 and message.udp_dst_port == 67 # and message.ipv4_daddr.to_s == "10.100.0.1"
            dhcp_in = DHCP::Message.from_udp_payload(message.udp_data)

            # logger.debug "DHCP: data:#{message.udp_data.unpack('H*')}."
            logger.debug "DHCP: message:#{dhcp_in.inspect}."
            logger.debug "DHCP: message:#{dhcp_in.to_s}."

            port = ports[message.in_port]
            return if port.nil? or port.network.nil?

            # Check incoming type...

            dhcp_out = DHCP::Offer.new
            dhcp_out.op = $DHCP_MSG_OFFER
            dhcp_out.xid = dhcp_in.xid
            dhcp_out.yiaddr = Trema::IP.new(port.ip).to_i
            # dhcp_out.chaddr = dhcp_in.chaddr
            dhcp_out.chaddr = Trema::Mac.new(port.mac).to_short + [0x00]*10 # Verify instead that discover has the right mac address
            dhcp_out.siaddr = Trema::IP.new(port.network.dhcp_ip).to_i

            dhcp_out.options.clear
            dhcp_out.options << DHCP::MessageTypeOption.new(:payload => [ $DHCP_MSG_OFFER ])
            dhcp_out.options << DHCP::ServerIdentifierOption.new(:payload => port.network.dhcp_ip.split('.').collect { | each | each.to_i })
            dhcp_out.options << DHCP::IPAddressLeaseTimeOption.new(:payload => [ 0xff, 0xff, 0xff, 0xff ])
            dhcp_out.options << DHCP::Option.new(:type => $DHCP_BROADCASTADDR,
                                                 :payload => port.network.broadcast.split('.').collect { | each | each.to_i })
            dhcp_out.options << DHCP::RouterOption.new(:payload => port.network.dhcp_ip.split('.').collect { | each | each.to_i })
            dhcp_out.options << DHCP::SubnetMaskOption.new(:payload => [ 0xff, 0xff, 0xff, 0x00 ])

            logger.debug "DHCP Offer: output:#{dhcp_out.inspect}."
            logger.debug "DHCP Offer: output:#{dhcp_out.to_s}."

            message_out = Trema::PacketOut.from(message)
            raise "PacketOut wrong protocol type." if not message_out.ipv4? or not message_out.udp?

            message_out.macsa = port.network.dhcp_hw.value
            message_out.macda = Trema::Mac.new(port.mac).value
            message_out.ipv4_saddr = Trema::IP.new(port.network.dhcp_ip).to_i
            message_out.ipv4_daddr = Trema::IP.new(port.ip).to_i
            message_out.udp_src_port = 67
            message_out.udp_dst_port = 68

            logger.debug "DHCP packet: source:#{message_out.macsa.to_s}:#{message_out.ipv4_saddr.to_s}:#{message_out.udp_src_port} dest:#{message_out.macda.to_s}:#{message_out.ipv4_daddr.to_s}:#{message_out.udp_dst_port}."
            logger.debug "DHCP packet: #{message_out.to_s_with_data(dhcp_out.pack).unpack('H*').inspect}."
            
            dhcp_out.options.each { |each|
              logger.debug "DHCP options: #{each.pack.unpack('H*').inspect}."
            }

            logger.debug "DHCP data: #{dhcp_out.pack.unpack('H*').inspect}."

            send_packet_out(datapath_id, :data => message_out.to_s_with_data(dhcp_out.pack), :actions => Trema::ActionOutput.new( :port => message.in_port ) )
          end
        end
      end
      ...
      def install_route port, hw, ip
        ...
        add_flow port, "priority=#{1}", "table=#{@@mac_route_table},dl_dst=#{hw}", "output:#{port}"

        add_flow port, "priority=#{2}", "table=#{0},in_port=#{port},dl_src=#{hw}", "resubmit(,#{@@route_directly})"
        add_flow port, "priority=#{1}", "table=#{@@route_directly},dl_dst=#{hw}", "output:#{port}"

        # @ovs_ofctl.add_flow "priority=#{1},table=#{@@load_dest_table},dl_dst=#{hw}", "load:#{port}->NXM_NX_REG0[],resubmit(,#{@@send_src_table})"
        add_flow port, "priority=#{1}", "table=#{@@load_dest_table},dl_dst=#{hw}", "drop"
      end

      def install_virtual_route network_id, port, hw, ip
        network = networks[network_id]

        add_flow port, "priority=#{7}", "table=#{0},in_port=#{port}", "load:#{network.id}->NXM_NX_REG1[],resubmit(,#{@@virtual_src})"
        add_flow port, "priority=#{1}", "table=#{@@virtual_dest},reg1=#{network_id},dl_dst=#{hw}", "output:#{port}"
      end
      ...
      def install_arp_antispoof port, hw, ip
        ...
        add_flow port, "priority=#{3}", "table=#{@@arp_antispoof_table},arp,in_port=#{port},arp_sha=#{hw},nw_src=#{ip}", "resubmit(,#{@@arp_route_table})"
        add_flow port, "priority=#{2}", "table=#{@@arp_antispoof_table},arp,arp_sha=#{hw}", "drop"
        add_flow port, "priority=#{2}", "table=#{@@arp_antispoof_table},arp,nw_src=#{ip}", "drop"

        # Routing of ARP packets to instance.
        add_flow port, "priority=#{2}", "table=#{@@arp_route_table},arp,dl_dst=#{hw},nw_dst=#{ip}", "output:#{port}"
      end

      def install_virtual_arp_route network_id, in_port, hw, ip, out_port, remote_hw, remote_ip
        network = networks[network_id]

        # Require correct ARP source IP/MAC from instance, and protect the instance IP from ARP spoofing.
        # add_flow port, "priority=#{3}", "table=#{@@virtual_dest},arp,reg1=#{network.id},arp_sha=#{hw},nw_src=#{ip},dl_dst=#{remote_hw},nw_dst=#{remote_ip}", "output:#{out_port}"
        # add_flow port, "priority=#{2}", "table=#{@@virtual_dest},arp,arp_sha=#{hw}", "drop"
        # add_flow port, "priority=#{2}", "table=#{@@virtual_dest},arp,nw_src=#{ip}", "drop"

        # Make generic...
        add_flow in_port, "priority=#{3}", "table=#{@@virtual_dest},arp,reg1=#{network.id},dl_dst=#{remote_hw},nw_dst=#{remote_ip}", "output:#{out_port}"
      ...
      def install_static_icmp icmp_type, icmp_code, port, local_hw, local_ip, src_ip
        ...
        incoming_actions = "load:#{port}->NXM_NX_REG0[],resubmit(,#{@@send_src_table})"
        add_flow port, "priority=#{3}", incoming_match, incoming_actions

        outgoing_match = "table=#{@@send_src_table},#{match_type},in_port=#{port},dl_src=#{local_hw},nw_src=#{local_ip}#{src_ip_match},"
        outgoing_actions = "output:NXM_NX_REG0[]"
        add_flow port, "priority=#{3}", outgoing_match, outgoing_actions
      end
      ...
      def install_static_transport nw_proto, port, local_hw, local_ip, local_port, remote_ip
        ...
        outgoing_match = "table=#{@@send_src_table},#{match_type},in_port=#{port},dl_src=#{local_hw},nw_src=#{local_ip}#{dst_match}"
        outgoing_actions = "output:NXM_NX_REG0[]"
        add_flow port, "priority=#{3}", outgoing_match, outgoing_actions
      end
      ...
      def install_virtual_static_d_transport network_id, nw_proto, in_port, local_hw, local_ip, out_port, remote_ip, remote_port
        network = networks[network_id]
      ...
      def install_local_icmp port, hw, ip
        ...
        add_flow port, "priority=#{1}", "table=#{@@send_src_table},#{match_type},in_port=#{port},dl_src=#{hw},nw_src=#{ip}", actions
      end
      ...
      def add_flow port, prefix, match, actions
        ports[port].active_flows << match
        ports[port].queued_flows << ["#{prefix},#{match}", actions]
      end

      def install_virtual_network network_id, dhcp_hw, dhcp_ip
        raise "Virtual network already created: id:#{network_id}." if not networks[network_id].nil?

        networks[network_id] = OpenFlowNetwork.new(network_id)
        networks[network_id].dhcp_hw = dhcp_hw
        networks[network_id].dhcp_ip = dhcp_ip
        networks[network_id].flood_flows << ["priority=#{1},table=#{@@virtual_dest},reg1=#{network_id},dl_dst=ff:ff:ff:ff:ff:ff", "", "output:<>", ""]

        @ovs_ofctl.add_flow "priority=#{1},table=#{@@virtual_src},reg1=#{network_id}", "resubmit\\(,#{@@virtual_dest}\\)"

        # Catch DHCP requests.
        @ovs_ofctl.add_flow "priority=#{2},table=#{@@virtual_dest},reg1=#{network_id},udp,dl_dst=#{dhcp_hw},nw_dst=#{dhcp_ip},tp_src=68,tp_dst=67", "controller"
        @ovs_ofctl.add_flow "priority=#{2},table=#{@@virtual_dest},reg1=#{network_id},udp,dl_dst=ff:ff:ff:ff:ff:ff,nw_dst=255.255.255.255,tp_src=68,tp_dst=67", "controller"

        logger.info "installed virtual network: network_id:#{network_id} dhcp_hw:#{dhcp_hw} dhcp_ip:#{dhcp_ip}."
      end

      def update_network network_id
        @ovs_ofctl.add_flows_from_list networks[network_id].generate_flood_flows
      end
    ...
    class OpenFlowNetwork
      ...
      def generate_flood_flows
        ...
        @flood_flows.each { |flow|
          flows << [flow[0], "#{flow[1]}#{generate_flood_actions flow[2]}#{flow[3]}"]
        }
        flows
      end

      def generate_flood_actions template
        actions = ""
