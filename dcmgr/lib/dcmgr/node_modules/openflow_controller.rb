  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def install_virtual_network network_id, dhcp_hw, dhcp_ip
        raise "Virtual network already created: id:#{network_id}." if networks.has_key? network_id

        network = networks[network_id] = OpenFlowNetwork.new(network_id)
        network.dhcp_hw = dhcp_hw
        network.dhcp_ip = dhcp_ip
        network.flood_flows       << ["priority=#{1},table=#{@@virtual_dest},reg1=#{network_id},reg2=#{0},dl_dst=ff:ff:ff:ff:ff:ff", "", "output:<>", ""]
        network.flood_local_flows << ["priority=#{0},table=#{@@virtual_dest},reg1=#{network_id},dl_dst=ff:ff:ff:ff:ff:ff", "", "output:<>", ""]
