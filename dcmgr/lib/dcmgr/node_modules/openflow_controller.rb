  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def initialize service_openflow
        ...
        networks[0] = OpenFlowNetwork.new(0)
        networks[0].flood_flows << ["priority=#{1},table=#{@@mac_route_table},dl_dst=FF:FF:FF:FF:FF:FF", "", "output:<>", ""]
        networks[0].flood_flows << ["priority=#{1},table=#{@@route_directly},dl_dst=FF:FF:FF:FF:FF:FF", "", "output:<>", ""]
        networks[0].flood_flows << ["priority=#{1},table=#{@@load_dest_table},dl_dst=FF:FF:FF:FF:FF:FF", "", "load:<>->NXM_NX_REG0[],resubmit(,#{@@send_src_table})", ""]
        networks[0].flood_flows << ["priority=#{1},table=#{@@arp_route_table},arp,dl_dst=FF:FF:FF:FF:FF:FF,arp_tha=00:00:00:00:00:00", "", "output:<>", ""]

        networks[1] = OpenFlowNetwork.new(1)
        networks[1].flood_flows << ["priority=#{1},table=#{@@virtual_dest},reg1=#{1},dl_dst=ff:ff:ff:ff:ff:ff", "", "output:<>", ""]

        networks[2] = OpenFlowNetwork.new(2)
        networks[2].flood_flows << ["priority=#{1},table=#{@@virtual_dest},reg1=#{2},dl_dst=ff:ff:ff:ff:ff:ff", "", "output:<>", ""]
      end

      def start
        logger.debug "OpenFlowController Thread Object Id: #{Thread.current}."
        logger.debug "Instances db: #{self.name}, #{Trema::App[self.name]}."
      end
      ...
      def features_reply message
        ...
        networks[0].add_port OFPP_LOCAL
        networks[0].add_port @eth_number
        ...
        @ovs_ofctl.add_flows_from_list networks[0].generate_flood_flows
      ...
      def install_virtual_route network_id, port, hw, ip
        ...
        add_flow port, "priority=#{1}", "table=#{@@virtual_src},in_port=#{port}", "resubmit(,#{@@virtual_dest})"
        add_flow port, "priority=#{1}", "table=#{@@virtual_dest},reg1=1,dl_dst=#{hw}", "output:#{port}"
      end
