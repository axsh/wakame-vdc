  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def install_route port, hw, ip
        ...
        @ovs_ofctl.add_flow "priority=#{1},table=#{@@group_outgoing_table},dl_src=#{hw}", "resubmit\\(,#{@@group_incoming_table}\\)"
        @ovs_ofctl.add_flow "priority=#{1},table=#{@@group_incoming_table},dl_dst=#{hw}", "output:#{port}"

        # @ovs_ofctl.add_flow "priority=#{1},table=#{@@load_dest_table},dl_dst=#{hw}", "load:#{port}-\\>NXM_NX_REG0\\[\\],resubmit\\(,#{@@send_src_table}\\)"
        @ovs_ofctl.add_flow "priority=#{1},table=#{@@load_dest_table},dl_dst=#{hw}", "drop"
      end
      ...
      def install_icmp port, hw, ip
        ...
        @ovs_ofctl.add_flow "priority=#{1},table=#{@@send_src_table},#{match_type},in_port=#{port},dl_src=#{hw},nw_src=#{ip}", "learn\\(#{learn_outgoing_match},#{learn_outgoing_actions}\\),learn\\(#{learn_incoming_match},#{learn_incoming_actions}\\),output:NXM_NX_REG0[]"
      end
      ...
      def install_transport_port nw_proto, port, hw, ip
        ...
        @ovs_ofctl.add_flow "priority=#{1},table=#{@@send_src_table},#{match_type},in_port=#{port},dl_src=#{hw},nw_src=#{ip}", actions
      end

      def add_flow port, prefix, match, actions
        @ovs_ofctl.add_flow "#{prefix},#{match}", actions
        port.active_flows << match
      end
    end
