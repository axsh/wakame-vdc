  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def features_reply message
        ...
        @ovs_ofctl.add_flow "priority=#{3},icmp", "resubmit\\(,#{@@load_dest_table}\\)"
        @ovs_ofctl.add_flow "priority=#{3},tcp", "resubmit\\(,#{@@load_dest_table}\\)"
        @ovs_ofctl.add_flow "priority=#{3},udp", "resubmit\\(,#{@@load_dest_table}\\)"
        ...
        # Temporarily allow DHCP;
        @ovs_ofctl.add_flow "priority=#{4},udp,tp_src=67", "resubmit\\(,#{@@mac_route_table}\\)"
        @ovs_ofctl.add_flow "priority=#{4},udp,tp_dst=67", "resubmit\\(,#{@@mac_route_table}\\)"
        @ovs_ofctl.add_flow "priority=#{4},udp,tp_src=68", "resubmit\\(,#{@@mac_route_table}\\)"
        @ovs_ofctl.add_flow "priority=#{4},udp,tp_dst=68", "resubmit\\(,#{@@mac_route_table}\\)"
        ...
        @ovs_ofctl.add_flow "priority=#{1},table=#{@@load_dest_table},dl_dst=FF:FF:FF:FF:FF:FF", "load:#{OFPP_FLOOD}-\\>NXM_NX_REG0[],resubmit\\(,#{@@send_src_table}\\)"
        @ovs_ofctl.add_flow "priority=#{1},table=#{@@load_dest_table},dl_dst=#{@local_hw.to_s}", "load:#{OFPP_LOCAL}-\\>NXM_NX_REG0[],resubmit\\(,#{@@send_src_table}\\)"
        @ovs_ofctl.add_flow "priority=#{0},table=#{@@load_dest_table}", "load:#{1}-\\>NXM_NX_REG0[],resubmit\\(,#{@@send_src_table}\\)"

        @ovs_ofctl.add_flow "priority=#{0},table=#{@@send_src_table}", "output:NXM_NX_REG0[]"
        ...
        #
        @ovs_ofctl.add_flow "priority=#{4},tcp,nw_dst=169.254.169.254,tp_dst=80", "resubmit\\(,#{@@metadata_outgoing_table}\\)"
        @ovs_ofctl.add_flow "priority=#{4},tcp,nw_src=#{Isono::Util.default_gw_ipaddr},tp_src=#{9002}", "resubmit\\(,#{@@metadata_incoming_table}\\)"
      ...
      def install_static_transport nw_proto, port, local_hw, local_ip, local_port, src_ip
        ...
        outgoing_match = "table=#{@@send_src_table},#{match_type},in_port=#{port},dl_src=#{local_hw},nw_src=#{local_ip}#{src_ip_match},tp_src=#{local_port}"
        outgoing_actions = "output:NXM_NX_REG0[]"
        add_flow port, "priority=#{3}", outgoing_match, outgoing_actions
      end
      ...
      def install_local_transport nw_proto, port, hw, ip
        ...
        learn_outgoing_match = "priority=#{2},idle_timeout=#{7200},table=#{@@load_dest_table},#{match_type},NXM_OF_IN_PORT[],NXM_OF_ETH_SRC[],NXM_OF_ETH_DST[],NXM_OF_IP_SRC[],NXM_OF_IP_DST[],NXM_OF_#{transport_name}_SRC[],NXM_OF_#{transport_name}_DST[]"
        learn_outgoing_actions = "output:NXM_NX_REG0[]"

        learn_incoming_match = "priority=#{2},idle_timeout=#{7200},table=#{@@load_dest_table},#{match_type},NXM_OF_IN_PORT[]=NXM_NX_REG0[0..15],NXM_OF_ETH_SRC[]=NXM_OF_ETH_DST[],NXM_OF_ETH_DST[]=NXM_OF_ETH_SRC[],NXM_OF_IP_SRC[]=NXM_OF_IP_DST[],NXM_OF_IP_DST[]=NXM_OF_IP_SRC[],NXM_OF_#{transport_name}_SRC[]=NXM_OF_#{transport_name}_DST[],NXM_OF_#{transport_name}_DST[]=NXM_OF_#{transport_name}_SRC[]"
        learn_incoming_actions = "output:NXM_OF_IN_PORT[]"
      ...
      def add_flow port, prefix, match, actions
        @ovs_ofctl.add_flow "#{prefix},#{match}", actions
