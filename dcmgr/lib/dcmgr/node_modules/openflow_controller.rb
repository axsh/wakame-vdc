  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def features_reply message
        ...
        message.ports.each do | each |
          ...
          if each.number >= OFPP_MAX
            ...
            install_eth port
            ovs_ofctl.add_flows_from_list port.queued_flows
      ...
      def install_eth port
        port.queue_flow "priority=#{6}", "udp,in_port=local,dl_dst=ff:ff:ff:ff:ff:ff,nw_src=0.0.0.0,nw_dst=255.255.255.255,tp_src=68,tp_dst=67", "output:#{@eth_number}"
        port.queue_flow "priority=#{2}", "in_port=#{@eth_number}",  "resubmit(,#{@@route_directly})"
        port.queue_flow "priority=#{0}", "table=#{@@mac_route_table}", "output:#{@eth_number}"
        port.queue_flow "priority=#{0}", "table=#{@@route_directly}", "output:#{@eth_number}"
        port.queue_flow "priority=#{0}", "table=#{@@load_dest_table}", "load:#{@eth_number}->NXM_NX_REG0[],resubmit(,#{@@send_src_table})"
        port.queue_flow "priority=#{4}", "table=#{@@send_src_table},in_port=#{@eth_number}", "output:NXM_NX_REG0[]"
        port.queue_flow "priority=#{1}", "table=#{@@arp_antispoof_table},arp,in_port=#{@eth_number}", "resubmit(,#{@@arp_route_table})"
        port.queue_flow "priority=#{0}", "table=#{@@arp_route_table},arp", "output:#{@eth_number}"
        port.queue_flow "priority=#{4}", "table=#{@@metadata_outgoing_table},in_port=#{@eth_number}", "drop"
      end
