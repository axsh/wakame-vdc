    ...
    class OpenFlowController < Trema::Controller
      ...
      def packet_in datapath_id, message
        ...
        if message.arp?
          ...
          return if port.network.nil? # or message.arp_oper != 1

          out_packet = Trema::PacketOut.from message

          out_packet.macda = message.macsa.value
          out_packet.macsa = port.network.dhcp_hw.value

          out_packet.arp_oper = 2
          out_packet.arp_tpa = out_packet.arp_spa.to_i
          out_packet.arp_spa = Trema::IP.new( port.network.dhcp_ip ).to_i

          out_packet.arp_tha = out_packet.arp_sha.value
          out_packet.arp_sha = port.network.dhcp_hw.value

          logger.debug "ARP out_packet; port:#{message.in_port} source:#{out_packet.arp_sha.to_s}:#{out_packet.arp_spa.to_s} dest:#{out_packet.arp_tha.to_s}:#{out_packet.arp_tpa.to_s}."
          send_packet_out(datapath_id,
                          :data => out_packet.to_s,
                          :actions => Trema::ActionOutput.new( :port => message.in_port ) )
          return
        end
        ...
        if message.ipv4? and message.udp?
          ...
          if message.udp_src_port == 68 and message.udp_dst_port == 67 # and message.ipv4_daddr.to_s == "10.100.0.1"
            ...
            dhcp_out.options << DHCP::BroadcastAddressOption.new(:payload => port.network.broadcast.split('.').collect { | each | each.to_i }) unless port.network.broadcast.nil?
            # Host name 'abcdefgh'.
      ...
      def install_virtual_route network_id, port, hw, ip
        ...
        port.queue_flow "priority=#{1}", "table=#{@@virtual_dest},reg1=#{network_id},dl_dst=#{hw}", "output:#{port.port_info.number}"
      end
      ...
      def install_virtual_arp_route network_id, in_port, hw, ip, out_port, remote_hw, remote_ip
        ...
        # Require correct ARP source IP/MAC from instance, and protect the instance IP from ARP spoofing.
        # port.queue_flow "priority=#{3}", "table=#{@@virtual_dest},arp,reg1=#{network.id},arp_sha=#{hw},nw_src=#{ip},dl_dst=#{remote_hw},nw_dst=#{remote_ip}", "output:#{out_port}"
        # port.queue_flow "priority=#{2}", "table=#{@@virtual_dest},arp,arp_sha=#{hw}", "drop"
        # port.queue_flow "priority=#{2}", "table=#{@@virtual_dest},arp,nw_src=#{ip}", "drop"
      ...
      def install_virtual_network network_id, dhcp_hw, dhcp_ip
        ...
        @ovs_ofctl.add_flow "priority=#{2},table=#{@@virtual_dest},reg1=#{network_id},arp,nw_dst=#{dhcp_ip}", "controller"
