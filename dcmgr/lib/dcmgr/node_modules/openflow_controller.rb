  ...
  module NodeModules
    ...
    class OpenFlowController < Trema::Controller
      ...
      def initialize service_openflow
        ...
        @ovs_ofctl = OvsOfctl.new service_openflow.node.manifest.config.ovs_ofctl_path
      end
      ...
      def features_reply message
        ...
        message.ports.each do | each |
          ...
          if each.number == 1
            ...
            @service_openflow.worker_thread.pass {
              @service_openflow.add_openflow_by_nic each.name
            }
      ...
      def port_status message
        ...
        when OFPPR_ADD
          @service_openflow.port_status_map[ message.phy_port.number ] = {
            :is_active => true,
            :nic_id => message.phy_port.name,
            :last_ipv4 => nil
          }

          @service_openflow.add_instance message.phy_port
        when OFPPR_DELETE
          if @service_openflow.port_status_map.has_key? message.phy_port.number
            @service_openflow.port_status_map[:is_active] = false
          end
      ...
      def packet_in datapath_id, message
        ...
        if message.ipv4? and message.tcp?
          ...
          if message.ipv4_daddr.to_s == "169.254.169.254" and message.tcp_dst_port == 80
            ...
            # send_packet_out(datapath_id, :packet_in => message, :actions => 
            #                 [ Trema::ActionSetDlDst.new( :dl_dst => @local_hw ),
            #                   Trema::ActionSetNwDst.new( :nw_dst => Trema::IP.new( Isono::Util.default_gw_ipaddr ) ),
            #                   Trema::ActionSetTpDst.new( :tp_dst => 9002 ),
            #                   Trema::ActionOutput.new( :port => OFPP_LOCAL ) ] )
          end
        end
    ...
    class OvsOfctl
      ...
      def initialize ovs_ofctl_path
        @ovs_ofctl = ovs_ofctl_path
      end
      ...
      def del_flow flow_match
        command = "#{@ovs_ofctl} del-flows br0 #{flow_match}"
