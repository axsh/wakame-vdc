...
module Dcmgr
  module NodeModules
    class NatboxCollector < Isono::NodeModules::Base
      ...
      def get_alive_network_routes
        Models::NetworkRoute.alives.map do |network_route|
          network_route_to_hash(network_route)
        end
      end

      def network_route_to_hash(network_route)
        [:id, :outer_ipv4, :inner_ipv4].each_with_object({}) do |key, hash|
          hash[key] = network_route.send(key)
        end
      end
    end
  end
