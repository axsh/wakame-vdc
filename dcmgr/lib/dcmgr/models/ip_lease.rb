  ...
  class IpLease < BaseNew
    ...
    def self.lease(network_vif, network)
      ...
      addrs = network.ipv4_u32_dynamic_range_array - 
        reserved - network.network_vif_ip_lease_dataset.alives.for_update.all.map {|i| IPAddress::IPv4.new(i.ipv4).to_u32 }
      ...
      NetworkVifIpLease.create(:ipv4=>leaseaddr.to_s, :network_id=>network.id, :network_vif_id=>network_vif.id, :description=>leaseaddr.to_s)
    end
  end
end
