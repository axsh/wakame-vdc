...
function deploy_vmfile() {
  ...
  [ -f ${local_store_path}/${vmfile_basename} ] || {
    cd ${local_store_path}
    [ -f ${vmfile_basename}.gz ] || ${work_dir}/dcmgr/script/parallel-curl.sh --url=${vmfile_uri} --output_path=${local_store_path}/${vmfile_basename}.gz
...
[ -f /etc/redhat-release ] && {
  ...
  [ -z "${range_begin}" ] && range_begin=`ipcalc -n ${ipaddr}/${prefix_len} | sed 's,.*=,,'`
  [ -z "${range_end}"   ] && range_end=`ipcalc -b ${ipaddr}/${prefix_len} | sed 's,.*=,,'`
} || {
  # ubuntu
...
for i in $(seq 1 ${hva_num}); do shlog ./bin/vdc-manage resourcegroup map hng-shhost hn-demo${i}; done
for i in $(seq 1 ${sta_num}); do shlog ./bin/vdc-manage resourcegroup map sng-shstor sn-demo${i}; done
