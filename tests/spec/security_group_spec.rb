  ...
  describe "/api/netfilter_group" do
    ...
    cfg[:groups_to_create].each_with_index { |group,i|
      it "Should create group #{group[:name]}" do
        @@res[i] = APITest.create('/netfilter_groups.json', group)
    ...
    cfg[:groups_to_create].each_with_index { |group,i|
      it "Should update data of group #{group[:name]}" do
        APITest.update("/netfilter_groups/#{@@res[i]["uuid"]}", {:description=>"#{group[:description]}(new)", :rule => cfg[:update_rule]}).success?.should be_true
      end
    }
    
    cfg[:groups_to_create].each_with_index { |group,i|
      it "Should delete group #{group[:name]}" do
        APITest.delete("/netfilter_groups/#{@@res[i]["uuid"]}").success?.should be_true
      end
    }
  end
end
