...
Then /we (.*) be able to show the new users/ do |outcome|
...
Then /we (.*) be able to create users with the same uuids/ do |outcome|
  @user_uuids.each { |uuid|
...
Then /we (.*) be able to associate the users with the accounts/ do |outcome|
