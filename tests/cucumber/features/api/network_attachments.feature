...
Feature: Network Port Attachments API
  ...
  Scenario: Detach and attach from a running instance
    ...
    When we detach from network "nw-demo1" port <vif:port:uuid> its current attachment
    ...
    When we attach to network "nw-demo1" port <vif:port:uuid> the attachment <vif:uuid>
  ...
  Scenario: Start an instance with a detached port
    ...
    And we detach from network "nw-demo1" port <vif:port:uuid> its current attachment
    And we make an api get call to instances/<instance_uuid> with no options
      Then the previous api call should be successful
      And from <vif:> take {"ipv4":{"address":}} and save it to <vif:ipv4>
