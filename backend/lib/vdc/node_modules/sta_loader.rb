...
module Vdc
  module NodeModules
    class StaLoader < Isono::NodeModules::Base
      ...
      def create(values)
        job = VolumeContext.new(values[:volume_id])
        rpc = Isono::NodeModules::RpcChannel.new(node)
        sdata = rpc.request('sta-collector', 'get_snapshot', values[:snapshot_id]) unless values[:snapshot_id].nil?
        data = rpc.request('sta-collector', 'get_volume', values[:volume_id])
        job.stm.state=data[:state].to_sym
        job.stm.on_create

        if sdata
          v = `zfs receive #{data[:pool_name]}/#{data[:export_path]} < /home/axsh/#{sdata[:account_id]}/#{sdata[:uuid]}.img`
          if $?.exitstatus != 0
          ...
          v = `zfs create -V #{data[:size]}m #{data[:pool_name]}/#{data[:export_path]}`
          if $?.exitstatus != 0
            logger.info("volume already exists")
            job.stm.on_fail
          end
        ...
        job.stm.on_register
        vr = `zfs shareiscsi=on #{data[:pool_name]}/#{data[:uuid]}`
        # if $?.exitstatus != 0
        #   logger.info("failed iscsi target request")
        #   job.stm.on_fail
        # end
        il = `iscsitadm list target -v #{data[:pool_name]}/#{data[:uuid]}`
        if $?.exitstatus != 0
          job.stm.on_fail
        end
